...
> **Repo**: This is the **OG single-terminal version** (`amacrf` / branch `MATS_Terminal`).
> The [`MATS`](https://github.com/wyc-dev/MATS) repo (`mats_backend`) is the newer backend that serves [`mats_app`](https://github.com/wyc-dev/mats_app) (client-side UI/UX).
> Relationship: `amacrf` (OG terminal) → `mats_backend` (new backend) → `mats_app` (client UI/UX).
