...
| **v2.0.32** | HL signing rewrite (phantom agent EIP-712 + msgpack + recovery bit); xyz DEX asset index offset (110000); `updateLeverage()` before order placement (fixes 40x→10x); SL/TP direction fix for short positions (stale local mirror → immediate trigger); `syncExchangePositions()` removes stale exchange mirrors; `syncSLTP()` validates HL position side + entry; `placeOrder()` only returns success on `filled` (not `resting`); UI filters stale positions in real mode; REAL/PAPER label based on `agentId` only; `getOpenOrders()` parses `limitPx` + `reduceOnly`; `PERP_DEX_NAMES` fix (`dex: 0` → `''` — HL API rejects number); `formatPrice()` price-magnitude-based decimals (BTC=0, ETH=1, SPCX=2, SOL=3, ATOM=4, DOGE=6) strips trailing zeros; `updatePosition()` skips `checkPositionExits()` for `agentId='hyperliquid-real'` (exchange SL/TP managed natively by HL trigger orders) |
