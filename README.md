...
A dedicated Skeptics agent stress-tests every position against historical experience data. The system self-evolves via a **cognitive evolution pipeline** (v2.0.853: 15 active layers + 1 Edge Validation + 1 Q-RL Alpha Discovery + 1 DCS v2 Risk Profile Differentiation + Self-Aware Evolution — 4 dead components pruned). The pipeline learns: online logistic regression (OLR) → shadow trading → first-passage path-risk → EM cycle chains → genetic algorithms → RIL pattern clustering → **Numeric Autoencoder** (learned non-linear market embedding) → **AttnRes cycle-history retrieval** (Kimi K3 attention residual transfer) → **dual pseudo-query specialization** (decision vs execution) → **anti-pattern memory** (failure lesson clustering) → **conditional WR soft gate** (code-level enforcement) → **combo WR gate** (symbol×side×regime Wilson LB) → **OLR P(win) × consensus discount** (multiplicative confidence scaling) → **execution-lens SL/TP** (stop-out-trained direct SL/TP control) → **experience replay buffer** (prioritized mini-batch retrain) → **close-context-aware learning** (closeReason + slNarrowed weighted learning) → **Plan G dynamic threshold** (5-factor hysteresis [45-55%] + multiplicative penalty decay) → **⭐ Edge Validation layer** (v2.0.833: alpha "lie detector" — quantifies whether each signal has genuine statistical edge via Sharpe/Sortino/bootstrap/DSR/walk-forward) → **⭐ Q-RL Alpha Discovery** (v2.0.835: first component that can DISCOVER new alpha — 270-cell Q-table with ε-greedy exploration, Wilson score LB, stationary bootstrap p-value, Benjamini-Hochberg FDR correction; Factor-Tagged Aligned Shadow follows LLM consensus with agent vote metadata) → **⭐ DCS v2 Risk Profile Differentiation** (v2.0.836: three risk profiles make truly different decisions — Aggressive gets continuous boost from Q-RL discovery confidence, Conservative only accepts high-confidence discoveries, Moderate is the fixed standard; continuous [0,1] DCS score replaces discrete tiers with 5 evidence dimensions + time decay + Edge cross-validation + recent performance + negative Q gate) → **⭐ ANN Index** (v2.0.843: IVF + spherical k-means approximate nearest-neighbour search over 384-d MiniLM embeddings — EXP vector memory scales to 10k records at ~12% of brute-force scan rate) → **⭐ Asset-Aware Meta-Learner** (v2.0.843: 3-level hierarchy symbol → category → global, each asset learns its own feature-pattern independently — low volume ≠ unreliable) → **⭐ Component Attribution + LLM-vs-Stats A/B** (v2.0.844-848: proxy credit assignment measures which component actually adds edge; statistical shadow trades against the LLM shadow in the same conditions to isolate the LLM's marginal value) → **⭐ Smart SL/TP + MFE calibration** (v2.0.852: institutional priority chain S/R → 50-candle 頂底 → ATR floor; **leverage-aware SL floor** so a 10x position isn't stopped out by a 0.81% structural SL; **MFE calibrator** derives TP target/cap + SL floor from real 1h/5m candle price-extension distributions, direction-aware for BUY vs SELL) → **closeReason integrity + closeTrade dual-mode guard** (v2.0.851-853: TradeRecord.closeReason populated end-to-end so learning weights are correct; `closeTrade` no longer silently skips position exits in dual mode; `closePosition` uses the actual HL fill price instead of a stale WS tick).
...
│     15 active layers + 1 Edge Validation + 1 Q-RL, v2.0.835) │
...
│     Calibrator + Self-Improver + Causal Reasoner +           │
│     Meta-Learner (Brier/ECE calibration, Thompson bandit,    │
│     paired-shadow uplift, adaptive α + curriculum)           │
