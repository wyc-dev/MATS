...
> ⚠️ **Deprecated / disabled components (v2.0.873-P9 status)** — Statistical validation confirmed noise, defaulted OFF (env reversible): **OLR hard gate** (ρ=+0.02 noise, soft multiplier preserved) / **Q-RL expectancy gate** (ρ=+0.0064) / **First-Passage multiplier** (claimed≥95% actual WR 39.1%) / **Momentum-OLR conflict gate** (0 trigger). **Paused**: active-exploration (UCB), Bayesian OLR, bStocks trading (P80-bstocks-hide - no money, service layer reserved). **Removed**: temporal-attention/cross-symbol-backbone/reward-shaping/world-model/risk-profile-edge-store (MiniLM)/dcs-calculator. **The only predictive entry feature = entryShadowWinRate (ρ=+0.106/+0.1378)** - Statistics (shadow 24h decay) determine the direction lean; LLM only does catalyst narrative supplement.
