...
> **狀態**: ✅ 生產級（production-hardened）。v2.0.135 用 **OLR (Online Logistic Regression) + Shadow Trading + First-Passage Probability** 取代 v2.0.122 嘅 RBC (Range-Based Clustering)。RBC 嘅 growing-hyperrectangle 設計有致命缺陷（boxes 只擴張唔收縮 → 最終所有 dimensions overlap → 永久 NO_EDGE；見 §B.9）。新系統改為 per-symbol / per-side 嘅 logistic regression，從真實 path-risk 結果（TP-before-SL）學習 P(win)。
...
每個 cycle (5 min):
  ...
  2. First-Passage（即時）→ P(TP before SL) from volatility + drift + SL/TP distances
     ↑ 覆蓋 cycle 1（OLR 未暖時）
  3. OLR Engine → query P(win) per side from shadow + paper + real outcomes
     ↑ cycle 2 起可用（backfill / live samples 累積後）
...
- Per-symbol, per-side (LONG/SHORT) online logistic regression
- Welford z-score normalization，**每個 feature 獨立計數**（per-feature `welfordCount[]` + `welfordMask`）——缺失 feature（例如冇 S/R pivot）返回中性 z=0，唔污染成個模型
- SGD online updates，source-weighted：real=4, paper=2, shadow=1, **backfill=0.3**
- `backfill` source 從 SGD decay 排除：`liveSamples = nSamples − backfillSamples`，200 個 backfill samples 唔會凍結模型
- Cold / stale / warm detection（`newestSampleTs`）：stale（>6h）→ `resetSymbol()` + 重新 backfill
- NaN guards on normalize()；confidence = high(≥50) / medium(≥20) / low(<20) samples
...
- **Multi-candle hold**：每個 shadow 向前掃描最多 `maxHoldCandles`（20），喺第一根觸及 SL/TP 嘅 K線 resolve；未 resolve 嘅 skip（唔偽造 label）
- H/L tracking from candle data；`srDistanceBps` = 到最近 S/R pivot 嘅實際距離
- 學嘅係 TP-before-SL（真實交易可盈利性），唔係 5 分鐘價格方向

**Layer 1c — First-Passage (`first-passage.ts`)**
- 即時 P(TP before SL)，from volatility (σ) + log-drift (ν) + per-side SL/TP distances
- Cox & Miller (1965) first-passage formula for Geometric Brownian Motion
- **RR-aware**：P(win) 對 breakeven probability（唔係 flat threshold）；per-side SL/TP（LONG + SHORT 獨立）
- 正確公式（v2.0.135 修正）：
  - LONG: P = (e^(2νa/σ²)−1) / (e^(2νa/σ²)−e^(−2νb/σ²))
  - SHORT: P = (1−e^(−2νa'/σ²)) / (e^(2νb'/σ²)−e^(−2νa'/σ²))
  - zero-drift limit = a/(a+b)
- vol 太低（<0.1%）→ 返回 50%（no edge）

**Cold-Start Backfill (`olr-backfill.ts` — 新檔案)**
- 首次 cycle per market：replay 歷史 Hyperliquid candles（186 根 M5）入 OLR 作 `backfill` source
- **Non-blocking**（`void ... .catch()`）：交易 loop 繼續行；first-passage 覆蓋 cycle 1，OLR cycle 2 起可用
- Idempotent：已 warm（≥20 total samples）嘅 market skip。實測：3 markets 注入 945 samples，~1s

### v2.0.135 修正清單（數學審計）

| ID | 嚴重性 | 問題 | 修正 |
|----|:------:|------|------|
| C1 | 🔴 Critical | first-passage LONG/SHORT 公式互換 | 用 scale-function 推導重寫，LONG/SHORT 各自正確 |
| C2 | 🔴 Critical | 用 raw μ 而非 log-drift ν | 從 log-returns 推導 ν |
| M4 | 🟡 Med | SHORT SL/TP 距離無獨立傳遞 | per-side 距離 |
| H1-H4 | 🟠 High | Welford 單計數、缺 decay 排除、NaN 無防護、無 cold/stale 偵測 | per-feature 計數 + backfill decay 排除 + NaN guards + newestSampleTs |
| M1-M6 | 🟡 Med | shadow 偽造 label、單 K線 resolve、H/L 缺失等 | multi-candle hold + skip unresolved + candle H/L |
| L1-L3 | 🟢 Low | source 無分類、無 reset、無 confidence 旗標 | source-weight + resetSymbol + confidence |

### UI 更新（v2.0.135）
- Agent Cognition legend：「RBC & Sentiment Analyst」→「OLR & Sentiment Analyst」（描述重寫）
- Evolution panel first-passage：breakeven-aware（P(win) vs `breakevenPLong/Short`，顯示 edge pp）+ low-conf badge + per-side SHORT SL/TP
- Per-symbol OLR：新增 source-breakdown 行（shadow / paper / real / backfill 計數）
- 刪除 dead code `ui/src/RBCVisualizer.tsx`
- Backend symbols payload 加 `longSource` / `shortSource`

### 測試
- `tests/evolution-memory.test.ts`：41 tests passing（23 regression + 8 backfill/Welford/multi-candle/S-R + 10 其餘）
- `tsc --noEmit` clean · UI build clean · backend smoke 無 error

> ⚠️ 下方 §17（RBC Engine）保留為歷史審計記錄（audit trail），**已被本節取代**，唔再生效。`rbc-clustering.ts` 檔案仍存在但無被 import（dead code）。
...
17. [P0 — RBC Engine (Range-Based Clustering)](#p0--rbc-enginerange-based-clustering) ⚠️ **DEPRECATED v2.0.135 — 已被 OLR + Shadow + First-Passage 取代，見 §17b**
17b. [🆕 v2.0.135 — OLR + Shadow Trading + First-Passage + Cold-Start Backfill](#-v20135--olr--shadow-trading--first-passage--cold-start-backfill)
...
| **唔靠過去 P&L (v2.0.88)** | 過去 drawdown/losses 唔係拒絕交易嘅理由——OLR（v2.0.135 取代 RBC）持續學習，市況不斷變化 |
| **所有持倉有 OLR/S/R (v2.0.92, v2.0.135 更新)** | OLR（取代 RBC）+ S/R 數據為每個未平倉部位生成，唔只 active symbol |
...
│   │   │   ├── OLR & Sentiment Analyst (multi-symbol + Fear & Greed + OLR P(win) + First-Passage)
...
│   │       ├── Phase 1.8         # 🆕 v2.0.80 Skeptics 驗證 Meta-Agent entryThesis → 拒絕即 HOLD (v2.0.94: 已有持倉則跳過)
...
│   │   ├── olr-engine.ts          # 🧬 OLR Engine — Online Logistic Regression (per-symbol, per-side)
│   │   ├── shadow-trade-engine.ts  # 🧬 Shadow Trade Engine — simulated TP-before-SL tracking
│   │   ├── first-passage.ts       # 🧬 First-Passage Probability Calculator (Cox & Miller)
│   │   │   ├── OLREngine class       # Per-symbol, per-side logistic regression
│   │   │   ├── feedTrade()           # SGD update from shadow/paper/real outcomes
│   │   │   ├── query()               # P(win) per side + source breakdown + recency
│   │   │   ├── ShadowTradeEngine     # Simulated LONG+SHORT per cycle
│   │   │   ├── calculateFirstPassage() # Instant P(TP before SL) from vol + drift + S/R
│   │   │   └── save()/load()         # Persistence (olr-state.json + shadow-state.json)
│   │   │
...
| 4 | **OLR & Sentiment Analyst** | 0.25 | 0.10 | 2-8x | OLR (Online Logistic Regression) 專家 + **恐慌指數 (F&G)** + First-Passage path risk。OLR 從 shadow trade + real trade outcomes 學習 P(win) per side。🟢 P(win) > 60% → 增加信心。🔴 P(win) < 40% → 強烈反對入場。First-Passage P(TP before SL) 即時計算路徑風險。每個持倉獨立評估。|
...
| OLR & Sentiment Analyst | 「OLR P(win) is PRIMARY factor; Fear & Greed confirms/denies」 |
...
| 回撤 > 15% | 警告但唔強制平所有倉位（v2.0.88：OLR（v2.0.135 取代 RBC）持續學習，市況變化） |
...
| OLR & Sentiment Analyst | 制度+FnG判斷 | 制度驗證持倉 | 制度驗證持倉 | ... |
...
#### OLR & Sentiment Analyst (OLR P(win) + First-Passage + Fear & Greed)
...
| OLR & Sentiment Analyst | Default | kimi-k2.6:cloud | OLR P(win) + First-Passage + Fear & Greed 分析 |
...
> **目的**: 從 candle 歷史數據提取 pivot high/low + 心理整數位 + 訂單簿流動性聚類，為 Agent 提供價格水平感知。與 OLR & Sentiment Analyst 聯動——chaotic regime 自動降級 context。
...
| **OLR Engine** | Online Logistic Regression — per-symbol, per-side P(win) | 8 維特徵, per-symbol | `olr-engine.ts` |
| **Shadow Trading** | Simulated trades tracking TP-before-SL outcomes | per-cycle, per-market | `shadow-trade-engine.ts` |
| **First-Passage** | Instant P(TP before SL) from vol + drift + S/R | per-cycle | `first-passage.ts` |
...
### 第二層：RBC — Range-Based Clustering（範圍聚類） ⚠️ DEPRECATED v2.0.135

> 🚫 **本章節已過時**。v2.0.135 用 OLR + Shadow Trading + First-Passage 取代 RBC（見目錄 §17b）。RBC 嘅 growing-hyperrectangle 設計有 box-saturation 致命缺陷（§B.9）。以下內容保留為歷史審計記錄。
...
>
> **位置**: `src/evolution/olr-engine.ts` + `src/evolution/shadow-trade-engine.ts` + `src/evolution/first-passage.ts`
...
**Agent 指引**: OLR & Sentiment Analyst system prompt + context injection 加入 confidence label（high/medium/low）同使用指引——低信心 verdict 當弱提示，高信心 verdict 當強信號
...
| 檔案 | 用途 |
|:-----|:------|
| `src/evolution/olr-engine.ts` | OLR 引擎 — per-symbol, per-side logistic regression |
| `src/evolution/shadow-trade-engine.ts` | Shadow Trade 引擎 — simulated TP-before-SL tracking |
| `src/evolution/first-passage.ts` | First-Passage Probability Calculator |
| `src/evolution/trade-pattern-classifier.ts` | 整合 OLR query，backfill 時 feed trade |
| `data/evolution/olr-state.json` | 持久化 OLR state（weights per symbol/side） |
| `data/evolution/shadow-state.json` | 持久化 Shadow Trade state（open positions + recent results） |
...
- **OLR (Online Logistic Regression)**（v2.0.135 取代 RBC）：per-symbol/per-side logistic regression，Welford z-score，從 shadow+paper+real+backfill outcomes 學 P(win)。RBC（Range-Based Clustering，growing hyperrectangles）已 deprecated。
...
[system] ✓ LLM: OLLAMA
[system] ✓ Agents created [Fractal Momentum Sentinel, On-Chain Whisperer, OLR & Sentiment Analyst, Independent Risk Auditor, Meta-Agent]
...
| **😱 Fear & Greed Index** | ✅ 1.7.0 | **alternative.me FNG · OLR & Sentiment Analyst 整合** |
...
| **🐛 Per-Agent maxTokens** | ✅ 1.5.0 | **OLR & Sentiment Analyst 2048 tokens 防止 JSON 截斷** |
