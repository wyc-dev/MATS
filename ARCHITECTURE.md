...
**v2.0.870-P6 三方法入場質素 + 攻擊輪**: ① SL Floor（`RISK_STOP_LOSS_PCT` 0.008→0.015）;② Breakout 確認（`shouldSkipBreakoutEntry`——BUY 喺阻力位下方 < 50bps → skip）;③ OLR 硬閘（`checkOLRHardGate`——OLR P(win) < 30% → block）。⚠️ OLR 硬閘已由 P9-olr-audit（2026-08-28）取代——統計驗證證實噪音 → 默認 OFF，env `OLR_HARD_GATE='true'` 可逆。45 單 counterfactual 零贏單受影響，合併避免 +11.54% → -0.52% margin。攻擊輪：非原子寫入修復（4 組件 `save()` 改 `atomicWriteSync`）+ calibrator 空腹根因（4 個 persist 調用加入主 persist cycle）。**P6-fix**：EM-guided 方向 bias 修復——兩邊同分時用最近 5m candle 升跌做 fallback（`getRecent5mDirection`），唔再 fallback BUY。
...
MATS 嘅核心競爭力係**認知演化管線**（v2.0.868-P1P2: 15 active + 1 Edge Validation + 1 Q-RL Alpha Discovery + 1 Component Attribution + 1 PAEL Exit-Price Learner + 1 LLM World-Model Layer + 1 LLM Direction Verifier + 1 EV Filter + 1 Close-Decision Calibrator + 1 Profitability Analyzer + 1 Entry Quality System）——每筆交易結果都會餵回學習系統，系統唔係固定規則，而係一個會進化嘅認知引擎。v2.0.833 移除咗 4 個 0-inference 組件（temporal-attention / cross-symbol / reward-shaping / world-model）同暫停 active-exploration。v2.0.835 新增 Q-RL Alpha Discovery（首個可以發現新 alpha 嘅組件）+ Factor-Tagged Aligned Shadow。**v2.0.857 移除風險等級區別化（moderate-only）**——矩陣 3×3 → 1×3。**v2.0.858 解鎖 cycle 期間市場選擇**。**v2.0.859 移除零消費者組件 + 修復 Q-RL/OLR backfill 重複喂飼 + OLR calibration shrinkage**。**v2.0.860 三因子探索 + adaptive 歸一 + SE operator-conditioned context**（Frontis-MA1/OpenMLE-Evo）。**v2.0.870-P16/P17 Hybrid Penalty Decay + Runs Test τ 調製**（三層 OR 混合衰減打斷 death spiral + Wald-Wolfowitz 游程檢定 τ 9–18h 自適應）。**v2.0.870-P50-P77 Binance bStocks 平行交易系列**（Agentic Wallet + 數據源 + x402 + 自動 swap + 企業行動風險 + 動態 map + 倉位同步 + 攻擊硬化 + 本地儲存預設；⚠️ **P80-bstocks-hide 已暫停**——交易 call site 移除、UI 隱藏，服務層保留）。**v2.0.870-P78 方案 B 預測反轉點**（`reversal-point.ts` 即時市場結構判斷入場反轉風險——ATH/ATL 距離 U 形 + EntryTiming + 大陽燭後回落 + 蠟燭形態 + 動量減速 + S/R + 15m 分歧；soft gate high ×0.5；SKHX -14.7% 案例 score 0.75 HIGH、誤傷贏單 0/6；buildOLRBlock 誠實信心修復——backfill-only 標明 NO LIVE DATA；**E1 反轉點離場——MAE/MFE 原版**（主神洞察：MAE/MFE 係 per-symbol 即時結果，比 ATH/ATL 通用閾值準 8 倍；**主神裁決回滾收窄版**——收窄冇好處，避免少 17% 誤傷一樣 0）——`closeReason='reversal_point'` 全鏈 8 處、SL 止血（s1 0.8×mae + s2 1.5×mfe + s3 冇動能、holdMin ≥ 15 全局必要）+ TP 提早鎖利（MFE ≥ 0.5% + 回吐 ≥ 30%）、反事實 SL 避免 228.1% / 誤傷 0% + TP 改善 25.4% / 錯過 0%（200 筆 realTrades）；**E3 edge 行誠實標明**；攻擊輪 6 漏洞全修 + 攻擊輪 2（E1 MAE/MFE 毒輸入 4 漏洞全修——負數 mfePct/-Infinity pnl/Infinity mfePct/持久化污染，mfeValid guard）。**v2.0.870-P79 四窗驗證機制**（`checkFourWindowAlignment` 純函數——**4h+1h = 方向 gate**（大方向順先入）+ **15m+5m = 時機確認**——死貓彈（5m順+15m逆）& 兩窗都逆 → hard block / 兩窗都順 ×1.1 / 順勢回調 ×1.0；防止「TP 後 re-entry 倒蝕」——BTC 案例 TP +18.4% → 追高入場 → E1 平倉 -5.2%；驗證 2 批次重複——死貓彈/兩窗都逆 WR 33%/0% 都差；方向分清楚（buy/sell 鏡像）；env `MOMENTUM_ALIGN_GATE`；攻擊輪 15 測試全綠——冇漏洞；**P79-fix TradingView chart 每 cycle 全黑修復**——React cleanup destroy 後 guard return 嘅 bug——Effect 1 依賴改 `[timeframe, symbol]` + Effect 2 error 重試；**P79-fix2 closeReason reconciliation 覆蓋 bug**——onFills 時序競態——檢查 exitThesis 已設 → skip）。**v2.0.870-P81 per-symbol MAE/MFE SL/TP 校準**（`mae-mfe-sltp.ts` 純函數——主神洞察「Shadow 加 per-symbol MAE/MFE 必然更準」——驗證 SL 噪音止蝕 61%→20%（MAE p95 cap 6%）/ TP 可達性 29%→57%（MFE p50×0.8）；Shadow 整合（openShadowTrades 用 PAEL 校準）+ 真實交易整合（computeSmartSLTP 加 maeMfeP95——widen-only floor——主神批准影響所有真實交易 SL/TP）；攻擊輪 1 漏洞全修——mfeP50 極大 tpPct clamp 50%；**P81-fix System Pause button 修正**——UI 本地更新 systemPaused 即時切換 Play/Pause——log 文字「RBC engine」→「learning engines」；**P81-ui-green UI 橙色全面轉 Hyperliquid 綠色**——`--gold` #F5A623→#97fce4 + 硬編碼金色 rgba→綠色）。 **v2.0.870-P82 盈利提升系列**（**P82-backfill-fix** backfill 假成功 bug 修復——`backfillFingerprint` count+latestClosedAt+version——200 筆歷史即刻入 stats——vol_expansion ×0.7 降權即刻生效；**P82-time-decay** success-pattern 時間衰減——`recent` ring + 時間加權中位數——breakout 點估計 +2.37% boost 但時間加權 -2.07% 降權——最近 breakout 全蝕；**P82-reversal-e1e2** reversal-point 離場校準——`perSymbolMaeP50` s1 閾值 p50×2 cap 20% + `trendAligned` 趨勢支持 ×1.5——BTC/SKHX 唔再被誤傷；**P82-pael-real** PAEL 只計 real——shadow MAE=0 污染消除 + 時間衰減 τ=7 日 + weight cap 100；**P82-e1e3** per-symbol MFE 鎖利校準 + rolling window；**P82-combo-ev** Combo EV Gate——`avgEwmaPnlPct` = 0.5×avg + 0.5×ewma——正 → 唔降權、負 → 降權——EWMA half-life 120 cycles 10 個鐘——拒絕污染值——舊 Wilson LB 誤傷 7 個低 WR 高回報組合）。 **P82-cap40** Plan G Penalty Cap 0.30 → 0.40——11 個強負期望值組合 comboPenalty 0.50 更強壓制 ×0.60——netPenalty clamp 非負 + decayMultiplier clamp [0,1]——污染值防禦）。**v2.0.870-P80 成功類型分類**（`success-pattern.ts` + `success-pattern-tracker.ts`——主神洞察「認準成功 pattern 增大盈利」——順勢突破 +2.92% boost ×1.1 vs 低波動擴張/新聞/動量確認 -1.47% 到 -2.42% 降權 ×0.7；完整閉環——close → record → 統計（持久化 success-patterns.json）→ 入場 gate getMultiplier（soft）+ **HACP 接駁**（buildSuccessPatternBlock 注入 Meta-Agent & Skeptics context）；驗證校準後 +0.19pp；攻擊輪 3 漏洞全修；**P80-backfill 歷史數據初始化**——200 筆 realTrades 乘數即刻生效——idempotent backfillDone flag；**P80-bstocks-hide bStocks 全面隱藏**——主神裁決唔賺錢——移除交易 Cycle call site + UI 隱藏（Wallet TVL/Binance 區塊/橙色 tag/fetch/grid 3→2格）——服務層保留 + bStocks_module.md 更新——效能確認唔拖慢；**P80-pnl-fix PNL 報告「冇資料」修復**——預設 mode real + localStorage 持久化）。**v2.0.873-P9-th-archive 決策歷史永續歸檔 + PNL 時段**（2026-08-30）——trade-history 10000 條 ring buffer 已滿(只覆蓋 31 日, 每 cycle 1 條)——最舊 decision context(confidence/regime/thesis)被靜靜丟棄且 realTrades 無法重構;`TradeHistoryArchive`(append-only JSONL, 異步 fire-and-forget)eviction 前先歸檔再刪——核心 zero-impact;backfill 10000 條至 `data/archive/trade-history-archive.jsonl`;PNL 加 month3(90日)/year1(365日)時段 + PAPER/REAL 佈局調整。**v2.0.873-P9-edt-fix edge_report 已證偽源退出 + F1 死權重重新分配**（2026-08-30）——learnedEdge(OLR ρ=+0.02)/pathEdge(FP claimed≥95% 實際 39.1%)固定中性 0.5——edge_report 唔係 report-only(recommendation=skip 硬 hold matrix action);confidence 唔收 olr 樣本;攻擊輪 D1 HIGH: regime 非 string `.toLowerCase()` crash → typeof guard fallback unknown;F1 `reallocateWeights()` V2 按有效源原比例放大(死地板 30-40% 消除——實驗: 22 cells 零誤傷 + 蒙地卡羅動態範圍 ×1.501 + threshold 中心性 0.5 零偏);`EDGE_REALLOC=false` 回滾;mats_web_app label 同步(P(win) OLR → P(win))。**v2.0.873-P9-llpp-edt 輕虧損過早保護 + SL/TP 持久化 + closeReason 統一**（2026-08-30）——**P0-4**: `closePosition` 有填 SL/TP 字段但 serialize 白名單 + Snapshot type 漏咗 → realTrades 0/292 有 SL（開倉有、平倉冇斷層）;修 type/白名單/Supabase/migration 23 + backfill 292/292（92 真實 + 200 中位數）。**P9-llpp**: consensus close 輕虧損倉(-3%~0% margin)32% 過早——`src/analysis/light-loss-protection.ts` 純延遲模型（P9-defer 鏡像: SL 兜底 + 回升救返 + 到期再決定;「續跌確認 cut」實證係過度設計）;`computeLLPPMaxDefer()` per-symbol 過早率條件化;llppDeferrals side/openedAt 綁定 + pendingAnalyses patch + GOT 閉環;攻擊輪 26/26(5 真漏洞: NaN 注入/expired 循環/Map 殘留)。`P9_LIGHTLOSS_PROTECT=false` 回滾。**P9-edt**: closeReason 統一——鎖利全部 `exit_price_lock`(0.5),止血拆分 `reversal_point_exit`(0.3)。**Phase C(5m 出場)NOT DOING**——實驗推翻（盈虧混淆）。以下逐層詳述：**v2.0.873-P9-sllock exit-price-lock 單位 bug**（主神 2026-08-29「why only earn so little」——GOLD SELL MFE 3.0% 只賺 2.1%;根因雙重單位 bug: ①threshold floor `0.3` 應係 `0.003`（0.3% fraction,高 100 倍 → 鎖利 threshold 30%,正常單永遠唔觸發）②`liveMfe`(%)同 `converted.mfePricePct`(fraction)單位唔一致,Math.max 揀放大 100 倍值;GOLD 觸發係「兩個放大巧合通過」,鎖利點已回吐。修正: floor 0.003 + liveMfe/100 統一 fraction。驗證 3806 pass 零新增）。**v2.0.873-P9-attack-round4 攻擊輪四**（主神 2026-08-29「不擇手段攻擊 holdmin/shadowvoice」——A1 CRITICAL: holdmin `openedAt` 未來值 1e308 → negative holdMinNow → consensus close 無限延遲 DoS → 加 `>0 且 ≤now+5min` 合理範圍檢查;A2 CRITICAL: SHADOW LEAN/voice 大小階 match 失效（normalizeSymbol('BTC')='BTC' ≠ 'btc'）→ lowercase 統一——可能係 agents 一直冇 quote SHADOW LEAN 嘅根因。驗證 3803 pass 零新增）。**v2.0.873-P9-holdmin 過早 consensus 平倉保護**（主神 2026-08-29「SILVER buy/sell 都蝕錢,出場時機很差」——邏輯實驗先證後改:①SL floor 校準 counterfactual 證明降低 floor 誤傷大 winner（MAE>5% 贏單平均 +11.2%,淨 ΔPnL −97.9%）→ revert 錯誤方向;②SILVER −16% 解剖: SL=None（HL DEX 未 set SL 就被 consensus close）、4min 持倉 —— 真問題係「consensus 平倉過早,SL 未 set 就被平」;③<15min consensus close 15 單 −17.89%（12 單有 MFE≥0.5% 鎖利空間）;④持倉時間: >4h +1.29% vs <10min −0.30%。修正: `P9_HOLD_MIN_MINUTES`=15min（0=關閉 clamp [0,60]）新倉保護——<15min 唔俾 consensus close（SL hit/大蝕 >3% margin 照止血）;同時令 SL placement 有時間完成（修 SILVER −16% 裸奔）。驗證 3797 pass 零新增）。**v2.0.873-P9-shadowvoice Shadow 統計層 voice**（主神 2026-08-28 TG「8 資產 24 小時冇開倉,shadow & cycle 一直 run 但冇新倉」——診斷: shadow 引擎 60 倉全面 SELL-lean（btc SELL WR 86% EV +1.22%/dram 72%/gold 53% vs BUY 全負 EV）,但 agents context 冇 shadow 統計（淨係見到 OLR 已證偽/FP 0pp/Direction RED）→ 全 HOLD。修正: `buildShadowVoiceBlock()` 注入 per-symbol×side shadow WR/EV（decayed）+ 即時倉位方向,marketDesc directionHealthBlock 之後;純 context 唔 hard block。驗證 3794 pass 零新增）。**v2.0.873-P9-attack-round3 攻擊輪三**（主神 2026-08-28「不擇手段攻擊 sealarm + unblock 新 code」——A1 CRITICAL: `SELL_ALERT_ENABLED` env gate 缺失(文檔承諾可回滾但 code 冇,同 KLINE_BLOCK_ENABLED 對照組不一致)→ 新增 `sellAlertConfig` + `if (sellAlertConfig.enabled)` gate;A3: perSide pnl NaN 源頭防禦(`Number.isFinite(t.pnl)` 先累積——NaN ?? 0 = NaN 係 JS 陷阱);A2 RELATIVE LEAN startsWith 匹配、A4 15m fetch 失敗、A5 robustMomentumPct 極限全部安全確認。驗證 3789 pass 零新增）。**v2.0.873-P9-unblock agents 解鎖**（主神 2026-08-28「大半日冇開倉,根源解決咗未」——13 個鐘零開倉,7 agents 全 HOLD conf 0.69 一樣;根源三層: OLR 已證偽但 agents 仍信（見 OLR 兩邊 <50% 就 HOLD——全樣本 ρ=+0.02 零預測力;⚠️ 分桶 WR 係已成交 subset 有選擇偏差,唔係預測力證據）+ DIRECTION HEALTH 兩邊都紅（實測 SELL WR=0% BUY 33-67%）+ 今日多次 reload。修正: buildOLRBlock 標「⚠️ OLR 已證偽 ρ=+0.02——數字係噪音」+ DIRECTION HEALTH `🧭[RELATIVE LEAN]`（兩邊紅且差 ≥0.2 lean 高 WR 側,細倉嚴 SL）。驗證 3781 pass 零新增）。**v2.0.873-P9-sealarm 短線急跌 SELL 警報**（主神 2026-08-28「5min + 15m 級急跌觸發 is important」——診斷: 系統只睇 24h/4h <−1.5% 先觸發 SELL-SIGNAL,15m 急跌冇偵測 → BTC 跌 −0.85% 都 7/7 HOLD。邏輯實驗 96h 多 symbol: 15m 急跌 >0.3% 續跌率 67%、3 支 15m 內 short 利潤率 91-100%;關鍵 5m 續跌確認——有確認 3 支後 −30.7%（賺）/ 冇確認 +26.1%（反彈陷阱蝕）。實作: `CandleCache` 擴展 '15m' interval + `compute15mMomentumPct`/`compute5mMomentumPct` + `buildDirectionHealthBlock` async pre-fetch + 新 `⚡[SELL-ALERT]`（15m 動量 <−0.3% 且 5m 動量 <0）純 context 注入——唔 hard block,range 標「只可短線順勢見好即收」;env `SELL_ALERT_ENABLED=false` 回滾。驗證 3775 pass 零新增）。**v2.0.873-P9-attack-round2 攻擊輪二**（主神 2026-08-28「不擇手段攻擊最近修葺嘅 code」——A1/A2 CRITICAL: `EXP_TIME_DECAY_HOURS`/`EXP_TIME_CUTOFF_HOURS` 冇下限 clamp → env=1e-9 令 EXP pWin 全滅/冷啟動死 DoS → zod min(1h/24h) + runtime τ≥1h 且 cutoff≥2×τ 雙重 clamp;A7 MED: sweep 非原子寫 → `atomicWriteSync`。盈利提升分析: shadow WR confidence 加權證據不足（對勝負 ρ=0.011 vs PnL ρ=0.106,記錄率 74/269 全 <0.45）——按幻覺修正精神唔加新組件;盈利提升已由 lock-pipeline/exp-sweep/deadweight 三批實證改動完成。驗證 3772 pass 零新增）。**v2.0.873-P9-deadweight 失效 gate 停用**（主神 2026-08-28「停用所有失效組件」——counterfactual 269 單驗證 `scripts/p9-deadweight-counterfactual.ts`:fpMult 壓制 79% 單但被壓制 WR 48.8%≈全場 46.1% = 無分辨力（FP 已證偽 claimed≥95% 實際 39.1%）→ 停用;g1Mult 0/269 觸發 = 從未影響決策（依賴已證偽 OLR）→ 停用;causal counterfactual 證明有效（負 contribution 單 WR 31.4% vs 正 64.2% = 攔截真壞單）→ **保留**——先證後改,本座原本想停 causal 被 counterfactual 推翻。入場特徵全譜 Spearman 證實 entryShadowWinRate ρ=0.106 唯一有預測力,其餘全 ρ<0.05。env 可回滾: `FP_GATE_MULTIPLIER=false` + `MOMENTUM_OLR_CONFLICT_GATE=false`（新增開關）。驗證 3763 pass 零新增）。**v2.0.873-P9-exp-sweep 幽靈樣本自動清理**（主神 2026-08-28「每個 Cycle 都要自動清理幽靈樣本——BTC 4 日冇開倉」——鐵證：BTC 最後開倉 08-24,幽靈樣本 1990 條（btc buy WIN +5% 10min thesis 一字不差,id 全部唔同 = 重複記錄）;全樣本 WR 66.5%→40.7%（live 46.1%）、BTC buy pWin 98.5%→73.2%（live 56%）。三層修復：recordClose 加 tradeId dedup（無 id 用 `expFingerprint()` symbol|side|pnlPct|holdMin|thesis 前 40 字符）+ 每 cycle `sweepGhostRecords()`（index.ts totalCycles++ 後,重寫 disk + 重建 ANN）+ EXP record 存 tradeId 對齊 realTrades（269/269 配對）。衰減（E1）同 sweep（E5）雙管齊下——衰減 fade 舊樣本,sweep 清幽靈）。**v2.0.873-P9-exp-power EXP 樣本威力落實**（主神 2026-08-28「唔覺得學習系統有好好善用樣本威力」——五層斷層：1.8a gate 零輸出 silent pass / BTC buy 樣本 69% 舊時代綁架（76% WIN vs live 56%）/ 相似度 mean 0.155 閾值錯配 / anti-pattern 只消化 75% LOSS / EXP.md 零 render。E1 時間衰減：`EXP_TIME_DECAY_HOURS`=168h τ 加權 + `EXP_TIME_CUTOFF_HOURS`=336h hard cutoff——pWin 乘 exp(-Δt/τ),Wilson 用時間窗樣本;驗證 SKHX/SNDK/CL/MU pWin 向 live WR 收斂（BTC buy 例外 = 樣本 outcome 污染）。E2 gate 落實：`EXP_MATCH_THRESHOLD` 0.55→0.40（留一 98% match）+ 新 `[EXP gate]` log 唔准 silent pass。⚠️ 發現樣本 outcome 標籤污染（EXP real 1741 vs realTrades 269;08-25 後 WR 81% vs 46%——同 OLR 41% reconciliation 污染同源）——建議 E5 樣本標籤對齊）。**v2.0.873-P9-opengate 開單頻率修正**（主神 2026-08-28「需要確實真係開到單,唔好俾太多嘢阻住」——log 鐵證 21 次 `Exploration skipped (1/2 found: ATR)` 全部係 EM-guided BUY 方向已確定 + ATR edge 存在但 ≥2 門檻卡死;方向已由 priority chain 確定,edge 門檻只係 thesis 質素保證。修正兩層：edge 門檻 ≥2→≥1（ATR/OB/S-R/funding/dip 任何 1 個確定有效 edge 已夠開單）+ OLR/FP 已證偽源移離 edge 資格（ρ=+0.02/claimed≥95% 實際 39.1%——唔可以再「證明 edge」,保留 thesis 資訊性 context）。數據驗證：ATR edge 觸發率 94% WR 48% PnL +1.77%;非 OLR 驅動單 213 個 WR 49% PnL +1.18%;卡死機會成本 +1.77% PnL 解除）。**v2.0.873-P9-lock-attack 攻擊輪修復**（主神 2026-08-28「不擇手段攻擊啱啱修葺嘅 code」——10 攻 2 中：A1 threshold 極細 1e-9 → 任何浮盈即鎖（過度鎖利 DoS, CRITICAL）→ `PROFIT_LOCK_MARGIN_THRESHOLD_PCT` + `opts.thresholdMarginPct` 下限 clamp 0.1% [0.1,5];A6 `_pendingTrailingLocks` 冇 side/openedAt 綁定 → close→re-open 撞舊 pending 誤鎖新倉（CRITICAL）→ `PendingTrailingLock` 加 side/openedAt + 新 `isPendingLockStale()`（side/openedAt 唔 match 即棄, 容差 5min）+ set/檢查/清理三層防線 + `consensusCloseDeferrals` key 同步綁定;A4 `shouldTrailingLock` margin peak cap 1000%。盈利提升方案 C：consensus close 浮盈延遲門檻 1%→0.5% margin（`P9_CONSENSUS_DEFER_MARGIN_PCT` clamp [0.3,5]——consensus 蝕單 83% 曾浮盈 ≥0.5%, 重放救返 24 單;同鎖利管道正交, 低風險最多延遲 2 cycle + 止血優先）。驗證：攻擊測試 15/15 + 相關 64/64 + 全量 3752 pass 零新增 + 前向重放成效不變（WR 72.9%）。**v2.0.873-P9-lock-pipeline 鎖利管道 margin-basis 校準**（主神 2026-08-28「驗證絕對成效,之後先 fix」——驗證三層：269 喺解剖（WR 46.1% 但 PnL +1.53% 正期望值;145 蝕單 76% 曾浮盈 ≥0.3% margin = 離場問題非入場問題;最近 50 單 30/30 蝕單全部可救）+ 保守可達分析（θ=0.5% margin → WR 85.5%）+ 前向 candle 重放 `scripts/p9-lock-pipeline-replay.ts`（15m 燭逐支、無 look-ahead、269/269 覆蓋 → θ=0.5%: WR 46.1%→72.9%、PnL Δ+53.21%、誤鎖大 winner 0）。根因 = 鎖利門檻 **price-basis 0.5% = margin 5% @10x**（`shouldColdStartLock` `liveMfe<0.5` + L3 前置 `liveMfe>=0.5`）→ 71/110 可救單（margin MFE 中位數 2.68%）未到門檻漏走。修復：門檻改 margin-basis（`liveMfe × lev ≥ PROFIT_LOCK_MARGIN_THRESHOLD_PCT` 預設 0.5% margin, env 可調 clamp [0.01,5]）、`shouldColdStartLock` 加 leverage 參數（向後兼容垃圾→1）、L3 前置同改、A8 cap 保留 + lev 溢出/NaN/負當 1;幻覺修正/確認式鎖掛/SL 唔收窄不變;34/34 測試 + 全量 3737 pass 零新增）。**v2.0.873-P9-amplify 確定有效信號放大**（主神 2026-08-28 指令「幻覺修正同時增加盈利入場頻率——把確定有效的機制信號放大」——瓶頸診斷：`dipReversionSignal` 觸發方向後仍需 ≥2 edge 先開單，但 OLR/FP 已證偽，確定有效嘅 TIP-BUY 會被 `edgeElements < 2` 卡死 skip；且 `dipReversionBoost` 對 size 零放大。兩層放大（env `DIP_AMPLIFY_SIZE_BOOST=false` 回滾）：① dip 觸發時本身成為 edge element（`TIP-BUY 確定有效信號` / `SELL-rip 確定有效信號`——唔再等已證偽源同意 → 開單頻率↑）② `dipAmplifyMultiplier()` 純函數 size 放大 TIP-BUY ×1.5 / SELL-rip ×1.25（cap 2.0，乘入 shadowSizeMult）。幻覺修正不變式：放大唔改 `dipReversionSignal` 閘門邏輯（低位/高波動/trending_bear 照擋——測試強制）。驗證：15/15 + 全量 3735 pass 零新增）。**v2.0.873-P9 震盪市盈利頻率恢復**（主神 2026-08-28「需要確實真係開到單」——組件 1 **consensus close 浮盈延遲確認**：`unrealizedPnlPct ≥1%` 倉位 consensus close 延遲一 cycle（最多 2 次，`consensusCloseDeferrals` Map）——防 9-13 時代唔存在嘅「共識切走贏單」漏水（重放 12/18 反彈 +25.9pp vs 6/18 繼續跌 −16.2pp → 淨 +9.7pp）；SL hit/虧損倉唔延遲。組件 2a **dip-BUY exploration**：`dipReversionSignal`（exploration-direction.ts 純函數——range regime + σ≤0.3% + obImb≤−0.2 → dip-BUY 候選，重放 208 喺 +177.1pp +0.85/喺；Priority 6.5 接入 exploration priority chain；shadow-gate 豁免、OLR/四窗/5m/momentum 照擋）；dip-SELL 三個獨立樣本一致否定（−21.7pp/−9.7pp/−8.0pp）唔實施——HL 微觀結構「跌完必彈、彈完陰跌」，買 dip 有 edge、賣 rip 冇）。**v2.0.873-P9-olr-audit OLR 硬閘剷除**（主神質疑「我懷疑 OLR 嘅實際成效」——269 喺統計驗證：Spearman ρ(pWin,PnL)=+0.02 零相關 / ρ(pWin,勝負)=−0.16 輕微反預測；十等分檢定預測曲線崩壞（pWin 14% 桶實際 58%）；OLR<0.35 block 全史 +56.3pp vs 最近 50 −77.2pp 兩個樣本相反 = 噪音定義。毒源：41% 標籤 reconciliation 推斷 + 特徵管線跨 symbol 污染 + 門檻 in-sample fit。實施：`applyEntryConvictionGates` OLR<35% 閘 + `checkOLRHardGate`（exploration + per-symbol）默認 OFF（env `OLR_HARD_GATE='true'` 可逆）；pWin 軟乘數 + agent context 注入保留。收斂路線：乾淨標籤重訓 + live Spearman>0.2 先恢復）。**v2.0.873-P9-qrl-audit Q-RL expectancy gate 默認關閉**（主神質疑「Q-RL 係咪真正有效嘅交易質量驗證插件」——ρ(Q-RL expectancy, 實際PnL)=+0.0064 零預測力；三層結構缺陷：momentum bin 單位錯配（±0.5 門檻收 fraction → 維度 100% 死亡）、訓練數據 = shadow 回填（10.8:1 BUY 偏差 + MAE=0 污染）、24/270 cells 探索；`QRL_EXPECTANCY_GATE` 默認 false，Q-RL 發現機制保留純觀察）。**v2.0.873-P9-fine-tune Dip-BUY range 位置過濾**（209 喂 × 25×15m range 位置重放：高位 >0.65 118 喺 +133pp vs 低位 <0.35 43 喺 −9.2pp；Era 複檢 14-27 高位 +1.77 vs 低位 −1.92/喺——dip-BUY 嘅 edge 喺「賣壓被吸收喺高位」唔係「接低位刀」；`dipReversionSignal` 加 `rangePosition ≥0.5` 過濾，數據不足保守放行）。**v2.0.873-P9-sell-tune Dip-SELL 對稱實施**（主神指令「Sell 倉位都會盈利，唔好抹煞 SHORT」——51 喺 SELL 重放：高位 >0.65 賣 rip 17 喺 +7.3pp 53% vs 低位 <0.35 追跌 19 喺 −28pp 32%；對稱規則 `ob≥+0.2 + rp>0.65` / `ob≤−0.2 + rp>0.65` → dip-SELL，`rp<0.5` → null 雙向唔開）。**v2.0.873-P9-attack 攻擊輪**（16 攻 1 中——V1 HIGH **split-brain 炸彈**：consensus close defer 嗰個 cycle `_pendingAnalyses` 仍係 action='close' 無 execution metadata → 客戶端 AutoTrade 執行 close 而後端揸住 = 指令分裂——修復：defer 時同步 patch pendingAnalyses（consensus→hold + `metadata.execution{blocked:true, blockedBy:'gate'}`）；B4 負 volatility → null 一行防禦）。**v2.0.873-P9-tip-fix 信號語義修正**（主神質疑「LONG 點解用 buy-dip？唔係應該 buy-tip？」——四組合 × era 重放 269 喺：buy-tip 9-13 +23.2pp 63% / 14-27 +124.0pp 42% 唯一跨時代穩健 edge；buy-dip 9-13 73% → 14-27 25% dip edge 已死；log 標籤 DIP-BUY → TIP-BUY，信號邏輯不變）。**v2.0.873-P9-attrib 歸因管道擴展**（convLedger 19 乘數站位 → 開倉 stash `gateLedgerCache` → close 消費 → `recordAttribution` `componentId='gate:<名>'`——歸因 2→19 站位；`scripts/p9-attrib-validate.ts` per-gate Spearman `|ρ|<0.1` → NOISE-FLAG LOUD——失效閘可被發現停用，本 session −22.4pp consensus close / −13.3pp 四窗 SELL 都係靠呢類歸因發現）。**v2.0.873-P9-sell-range-gate**（commit message 聲稱 range 高位先賣 +0.43/喺；實際 diff 只加 `OLREngine.DISABLED = true`——**src/ 全樹冇引用點 = 死代碼**；實際 OLR 硬閘由 P9-olr-audit env `OLR_HARD_GATE` 控制默認 OFF；range 高位先賣已由 P9-sell-tune 實施。⚠️ 建議刪除未接駁 flag）。**v2.0.873-P9-verify 驗證輪**（主神 2026-08-31「先驗證再執行」——四輪獨立實驗零 code 改動, 831 §9-12）: ①大 winner 保護審計（38/39/40）——mom24-guard 誤傷 4 大 winner（+10%）但淨 +88pp 仍最優（維持 skip 版;「近支撐豁免」「SKHX per-symbol」全部推翻）;②順勢 confidence size up（41）❌——大 winner 係低概率高回報尾部,「+20pp OLR edge 應 size up」係已證偽建議源（ρ=−0.16 反預測）;③PLAN_5m-live-gate A + 支內逆勢（42/43）❌——DRAM 00:07 支內微跌=SELL 順勢（開倉時 live 已反彈嘅敘述唔準確——反彈喺 entry 之後）——開倉前信號 preempt 唔到開倉後反轉;④gate 誤傷成本審計（45）——mom24-guard 1:1.84 / chase-tail 1:2.77 / sr-size 1:2.02——「蝕少好多」先係主引擎。**v2.0.873-P9-reentry-cooldown 同 symbol 同向連蝕 hard cooldown**（主神 2026-08-31, DRAM 5 次 SELL 防禦）——兩輪實驗核心定理: 結構斷裂嘅可靠偵測器係「連蝕事件」而唔係「平均統計」（48-chase-effect per-symbol 14d lean Δ−40.4% ❌ / N=2 連蝕 6h block Δ+123.8% ✅）;三關全過（被抑制組 avg −2.48% / 剔 outlier 50/50 / 兩半 +73.5%/+42.3% / T=3~12h 全正）;實作 merge 入 `src/lib/reentry-cooldown.ts`（updateCooldownOnClose + shouldBlockChaseCooldown,attack-hardening clamp）+ index.ts 接駁 + onPositionClosedLearning;同現有「鎖利 cooldown」互補（exit_price_lock 後 1h 防贏單追返 vs 連蝕 6h 防蝕單追返）;env `REENTRY_COOLDOWN_GATE=false` 回滾。**v2.0.873-P9-reentry-attack 攻擊輪**（主神「不擇手段攻擊」,11 攻擊 1 真漏洞 CRITICAL 全修）: V1 `__proto__` prototype pollution 假造連蝕（`{__proto__:{streak:5}}` → streak=5 → 蝕 1 次變 6 → 假 cooldown 凍結開倉）→ 所有 state 讀取改 hasOwnProperty own-check（streak/cooldownUntil 兩處）;GOT 閉環接駁（gate='reentry-cooldown'）;**「過期後首單縮 size」推翻**——cooldown 過期後首單 avg +3.17% WR 61%（好過其他單 1.01%）——斷裂已過 6h,恢復交易正確。**v2.0.873-P9-boundary-align 開倉時序對齊**（主神「今日蝕咗好多錢」——DRAM 連蝕研究, PLAN_DRAM-exhaustion.md）——DRAM 4 層根因鏈（F1 median lagging 睇唔到最後 1-2 支轉向 + persistent_bear lean SELL + 5m gate 用已 close candle + mom24-guard 只 apply BUY）;T2 支尾延遲 ✅（支尾 4-5min avg −1.10% WR 35% vs 支中 +1.72% WR 62%,skip Δ+75%）→ `boundary-align.ts` `shouldDeferToBoundary()` 純函數 + executeTrade 接入（支尾 ≥4min → 延遲 ≤60s 至下支開頭,零 look-ahead/零離場干預）;T1 追跌尾/追升尾 ❌（期 1 誤傷 + SELL n=4 + symbol 兩極——5m close 層都 lagging,「支內 live 反彈」只有現場 tick 捕到 = PLAN_5m-live-gate A follow-up）。**v2.0.873-P9-verify-llm AI 預測能力驗證**（主神 2026-08-31,零 code）: LLM+live context 事前判斷 60 筆命中 47% < baseline 53%（開倉前資訊本質唔含未來 5min 反轉 + 開源 LLM 冇短期前瞻雙重限制）;HL API 歷史 tick 不可得（tradeHistory deserialization fail / recentTrades 只返 10 秒跨度）——Live tick 驗證唯一路徑 = 現場收集 + GOT 閉環;「反序」揭穿——低 shadowWR 「avg +2.22%」剔 13 筆大 winner 後 −1.04%（尾部撐起,學反序=學噪音）。**v2.0.873-P9-got-observe GOT 歸因修復 + 盈利調查**（主神 2026-09-01「先調查再執行」——四候選三否決 831 §13: swing-SL Δ−26~−46% 兼倖存者偏差教訓（64 全樣本推翻 63 初步）/ deep-consensus defer Δ−19~−61%（救細蝕大——deep loser thesis 已斷）/ SL-noise 非單調 / 美洲時段 BUY 兩半不穩 — 全部 ❌）+ **GOT 三層歸因缺陷全修**（「You can't tune what you can't attribute」）: entry gates 由 generic 'gate' bucket 改傳真 gate 名（blockedBy）；close-defer 記錄統一 source-site（direction='close'+side,滅雙重計數+語義污染）；flush 唔再記 finalAction='hold' rows；legacy 'gate' pending 丟棄；`summary()` per-gate 單行 + 每 100 cycle log。**v2.0.873-P9-got-observe-attack GOT 攻擊輪**（主神「不擇手段攻擊」,3 真漏洞全修）: V1 HIGH `save()` 非原子寫 → atomicWriteSync（其他組件早有,唯獨 GOT 漏）;V2 MED load() 1e308 污染 → n=Infinity/hitRate=NaN/comparator 不穩定 → clamp hits/misses ≤1e6、avgMovePct ≤±1e4;V3 LOW summary() NaN comparator → safe comparator + finite guard。**三問題驗證全屬實**: OLR 仲有投票權（olrBlendFactor 軟乘數保留,ρ=+0.02 已證偽但 counterfactual 證移除唔改善 PnL——A1/A2 都 ❌ 831 §14）+ 兩個 pwin 並存（signal 層 Shadow WR vs gate 層 OLR pwin）+ shape/convexity 埋喺 base（LEDGER 拆解已解決）。**v2.0.873-P9-ledger-shape-expose**（主神 2026-09-01）: B（純觀測零風險）conviction ledger `base(...)` 拆 shape/convexity 成獨立條目（effectiveConfidence 數值零變）;A1 OLR blend 中性化 ❌ / A2 Shadow WR 替換 ❌（E3 方向錯誤 + bnb 100% 集中 + outlier 驅動）——維持現狀;⚠️ 更正「OLR ≤50%」誤判（81 子集,全樣本 68.4%）——Simpson's paradox: symbol 層 ρ=+0.250 正確先驗 + within-symbol 噪音（−0.08~−0.33）——OLR 係「弱但正確嘅 symbol 層風險過濾器」。**v2.0.873-P9-shadow-sym-case Shadow WR case-sensitivity bug**（主神「點解 Shadow WR 得 btc+bnb」）: getStats() 返細階 vs normalizeSymbol() 保留資產名大小階（`xyz:SILVER` ≠ `xyz:silver`）→ 9/11 xyz: 資產 Shadow WR 靜默丟失（btc/bnb 100% match 先有）——「樣本太薄」結論係錯,係 bug;新 `ShadowTradeEngine.getStatsForSymbol()` case-insensitive + null 防禦,index.ts 12 call site 統一;歷史 entryShadowWinRate 無法 retroactive 修復（look-ahead 污染,需等新數據 2+ 週）。**v2.0.873-P9-sr-size-unify sr-size-gate 統一入 applyEntryConvictionGates**（方案 B——主神 2026-09-01,DRAM 18:45 SELL −8.0% 追跌尾）: sr-size-gate 原本只接 executeTrade（active 路徑）——per-symbol consensus/exploration bypass（同 v2.0.870-sell-seed-accel S0 統一執行路徑同款 bug）;移除 executeTrade 舊 block（防 double-shrink）→ applyEntryConvictionGates shadow-gate 後統一執行（三路徑都行）;SELL-only + 純 size 層 + 冷啟動保守放行 + env `SR_SIZE_GATE=false` 回滾;`shouldShrinkSrSize` 單一 source of truth;⚠️ per-symbol threshold 唔做（data-snooping 陷阱——SKHX n=3,831 §19）。**v2.0.873-P9-sym-sell-unblock**（主神 2026-09-02「BTC & BNB 咁耐無開倉」調查——三層斷裂點拆解 + fix #1+#2, 先證後改）: ①**#1 activeSymbol 注入覆蓋**——`activeSymbol = nonPositionMarkets[0]`（冇倉第一 = btc）但 agents marketTicker = selectedSymbol（UI 手動鎖定後 ≠ activeSymbol）;舊 `_additionalMarkets = nonPositionMarkets.filter(s => s !== activeSymbol)` 令 BTC 同時唔係 marketTicker 又唔喺 additionalMarkets → agents 完全見唔到 BTC（8 日, 實證 perSymbolConsensus 8 條無 btc、agents quote 7/8 無 BTC）;修復: `_additionalMarkets = nonPositionMarkets` 全保留（注入層 existingSyms dedup + HACP perSymbolMap Map merge 防重複）。②**#2 Shadow SELL 樣本餓死（死亡螺旋重現）**——5 條 shadow 開倉路徑上限全係「全 symbol 合計 10」,buy 佔滿 10 位後連 seeded sell 都 return（實證 open 56 buy/4 sell, btc/gold/sp500/skhx/bnb 全 10/0）;修復（方案 C）: 新 `countOpenBySide()` helper 5 路徑改 per-side 上限（buy/sell 各 10）+ `openSeededShadow` sell bypass per-symbol 上限（只受池 60 + cooldown）+ `evictOldestBlindForRoom(preferSide)` sell 優先 evict buy-blind;v2.0.861 不變式保留（blind 唔 self-evict——池滿直接 reject 最低優先）。資格不變式守住: seed sell 資格唔郁（persistent_bear + mom24h<0 + mom4h<0 雙確認——反彈型 BTC/BNB/GOLD 照舊唔 seed, E1 bounce 型 sell 全輸教訓）。驗證: RED 實驗重現兩 bug → GREEN 消除;新攻擊測試 12/12;全量 4011 pass + 13 pre-existing（零新增）;tsc clean。**v2.0.873-P9-sell-unblock-attack 攻擊輪**（主神 2026-09-02「不擇手段攻擊啱啱修葺嘅 code」——紅先 10 攻 7 命中全修）: V1×3 **side whitelist 統一**——openAlignedShadow/Statistical/QRL 原本冇 guard（對照 openSeededShadow 有,防禦不一致）→ 新 `isValidShadowInput()` helper 5 路徑接入（garbage side 'banana'/'hold' 穿過 per-side helper 開垃圾倉 = 污染 OLR 訓練）;V2×2 **load() sanitize**——garbage position（side=hold/symbol=''/SL Infinity）佔池 + 污染 per-side 計數 → 新 `sanitizePosition()`（drop + SL/TP default——A4 不變式 load 層都守）;V3×2 **空/空白 symbol** 5 路徑拒。V4-V6 通過。**盈利提升分析**: SELL entryShadowWinRate ≥0.55 唯一正 edge（n=8, WR 88%, EV +2.32%）——sell 樣本餓死修復正正令呢個信號有樣本流入;但 n=8 未夠開 gate（831 data-snooping 教訓）——2 週後再驗;BUY sl_tp −290%（48 單）已由 lock-pipeline/LLPP/holdmin 覆蓋,唔加第三層 block gate（P9-deadweight 教訓）。新測試 10/10;全量 4021 pass + 13 pre-existing（零新增）。**v2.0.873-P9-mfe-expose**（主神 2026-09-02「Why 某些資產無 MFE」+「gate:mae-pattern Expectancy +3.2% 但 Contrib −0.255」——先 PLAN + 邏輯實驗驗證先實作）: ①#1 **avgMfe 顯示修正**——`getStats()`（shadow-trade-engine.ts）open positions 只加分母冇入 MFE 分子 → 純 open 倉 symbol（SNDK/DRAM/SILVER 新開 shadow）UI 顯示 0.0% 誤導（實測 SNDK MFE 1.5%/DRAM 0.8%/SILVER 0.4%——實驗 A 實證）;修復: open positions（checkPositions 每 cycle tick/candle 極值——真實數據）計入 avg。②#2 **gate 歸因語義修正（兩層缺陷）**——gate mult（confidence 乘數）被當方向 signal → sell 單 agreement=1−mult 反轉 =「gate 反 SELL」（其實 0.85 = 收緊 SELL）→ 任何 soft gate 對 sell 單系統性錄負 contrib（bias, 同 gate 成效無關——實驗 B 實證）;第二層 signal clamp [0,1] 丟失加成資訊（×1.1 boost → 1.0）。修復（component-attribution.ts）: 新 `signalRaw` 字段（clamp 前 mult, load/save round-trip）+ `gate:*` contribution 用出手命中語義（收緊→蝕=避損啱正/賺=誤傷負;加成→賺=正/蝕=負;mult=1 中性 0）+ computeStats confident 用 |rawMult−1|≥0.01（出手先算）;non-gate（olr/causal）照舊 agreement 框架。**修正後真面目**: mae-pattern −0.255→−0.364（出手 11/11 收緊→trade 平均 +3.2% = 誤傷本應賺嘅倉 → P9-deadweight 候選）;base(...) +0.195→−0.500;四窗 mult=1.0 未出手 → 0（block 成效由 GOT counterfactual 量——正確分工）。新測試 8/8;全量 4029 pass + 13 pre-existing（零新增）。**v2.0.873-P9-mfe-expose-attack 攻擊輪**（主神 2026-09-02——紅先 10 攻 6 命中全修）: V1×3 **avgMfe/avgMae 入帳無 clamp**（mfePct=1e308 finite 過 isFinite → avg 巨大值;負數 avgMae 誤導——step1 open/step2 resolved/step3 recent 三處統一 `clampExcursion([0,0.5])` 極值比例合理範圍）;V2×2 **load() 持久化污染無 clamp**（contribution=5 → computeStats 平均撐爆;signal=−3 污染 expectancy——契約 [-1,1]/[0,1]）;V2c signalRaw string/Infinity → gate confident 判定污染（typeof+isFinite 防禦→undefined）;V3 **sanitizePosition 無 sanitize mfePct/maePct**（load 層源頭 clamp）。V4（pnl=0）/V4b（`gate:` 空名）/V5（mult=1.0000001 浮點噪聲 ε guard）通過。**盈利提升分析（修正後歸因層首次真實可用）**: 全部 6 soft gate（success-pattern/reversal-point/cal-trust/mae-pattern/base/convexity）誤傷率 68-80%（出手後 trade 平均賺 +1.9~+2.7% = 收緊本應賺嘅倉）——「gate 系統性過度保守」強候選;但樣本 10-19 未夠（deadweight 標準 269 單）→ 2-4 週樣本累積後 counterfactual。修正本身已係盈利提升: 歸因層乾淨——「You can't tune what you can't attribute」真正成立。新測試 10/10;全量 4039 pass + 13 pre-existing（零新增）。**v2.0.873-P9-edge-evolution System Engineer 解放**（主神 2026-09-02「我本身要 System Engineer 做嘅嘢唔係單純困於 831.md,831.md 只係例子,我係希望佢可以不斷演化對 edge 嘅觸覺」——兩層: SystemEngineer.md 授權 + harness 權限升級）: ①**SystemEngineer.md 授權 831 VERIFY-FIRST WORKFLOW**（Owner's Directive 最高優先權章節）——執行權限（寫 PLAN_*.md / 寫+執行 scripts/*-experiment.ts / 用實驗裁決 / 改 831.md append §N / 全樣本誠實重放）;五步流程（PLAN → 實驗 → 三關 → 裁決 → 實作+同步）;831 紀律（零 look-ahead / data-snooping 防線（within-symbol ρ + 剔最大 symbol）/ 樣本門檻 n≥15 / 已證偽源 OLR/FP/Q-RL 唔做 gate 基礎）;快速路徑（bug fix/顯示歸因層/攻擊輪）vs 強制 831 全流程（新 gate/組件/threshold/feature）;Scope/North Star/Execution Flow/Output Format（verifyFirst 字段）/Known Good/Rule 15/Anti-Patterns 全鏈同步。②**harness 解放（src/evolution/system-engineer.ts, +86/−7）**——Scope ALLOW 加 scripts/ + PLAN_*.md + 831.md + SystemEngineer.md + AGENT_PROMPT.md/ARCHITECTURE.md;FORBIDDEN 補 src/edge/（owner-controlled）+ CHANGELOG.md（harness 專責——SE 提供 changelogEntry）;新檔支持（apply 段 creatingNewFile——`oldCode='((new file))'` 建立 experiment script, mkdir recursive, 跳過 existsSync/oldCode match）;實驗執行能力（tsc 後 scripts/ 新檔實際 `npx tsx <file>` 120s timeout, head 800 字符記 log——寫→跑→睇結果閉環;fail 唔 rollback 探索性容忍）;EDGE EVOLUTION AUTHORITY prompt（831 只係底線,主動 hunt edge: shadow stats / per-side EV / gate 誤傷率 / 時段-condition EV / price 幾何特徵→ ρ sweep/分桶/三關→有預測力先實作）。保衛不變: src/trading/（執行/SL/TP）+ src/config/（風險）+ src/data/（feed）+ src/api-server.ts + .env 仍 FORBIDDEN——資本安全紅線冇解放;Five Absolute Rules 仍在 prompt 最高優先權。⚠️ 過程誠實記錄: 首次 edit 整出 catch-block 重複（tsc 過但 esbuild/vitest 炸, 26 個 v2.0.860 test collect fail）——即時修復, 最終與 stash 原版一致。全量 4039 pass + 13 pre-existing（零 regression）。**v2.0.873-P9-pnl-capture PNL 長頁面 capture/PDF**（PNL 時段系列後續）: 1 MONTH ~30000px 超 html2canvas 16384px 硬上限（全黑/空白/hang）;試錯 5 步（chunk revert/移除 windowWidth/動態 scale）→ **最終兩路方案**: 短頁面（≤10000px）保留 html2canvas+jsPDF（keep LOGO/QR）+ 長頁面（>10000px）用瀏覽器原生 window.print()（@media print 白底）——純 pnl.html UI,零測試影響。以下逐層詳述：
...
**⛔ v2.0.875-P9-qrl-pool-monopoly（2026-09-11）：Pool 壟斷修復 + per-side 配額**——真實病例: 已證偽 Q-RL(ρ=+0.0064)arm 壟斷 shadow pool(recentResults 200 筆 qrl 佔 99.5%、open 60/60 全 qrl-buy、sell=0)→ sell 樣本餓死 → agents 冇 lean → 錯過跌勢。三層修復:
- **A. per-side 配額**: `SHADOW_CONFIG.maxOpenPerSide=30`(buy/sell 各 30)——**樣本生成器(blind/qrl)受配額**, aligned/statistical/seeded(lean 驅動/播種)唔受。
- **B. evict 優先序**: `evictOldestBlindForRoom()` blind(最低)→ **qrl(已證偽可犧牲)** → aligned; seeded 永不 evict; barrier-hit 保護保留。
- **C. Q-RL arm 封頂**: `countQRLShadows(sym)` per-symbol ≤3 + `getQRLShadowCount()` 全局 ≤24(≈40% pool)。
- **攻擊硬化**: `allPositions()` sanitize helper——所有遍歷(.find/.some/.filter ×8)經佢(null element 持久化污染唔 crash); evict 即時 `indexOf`(唔用 cache idx——splice 後 shift); `canOpenBuy/canOpenSell` 接全局配額。
- **驗證**: `scripts/p15-sell-recovery-verify.ts`(pre-registered: sell:buy≥0.2 / qrl<40% / open sell≥1)——修復後幾小時重跑(主神「每 3 分鐘 cycle」)。
**🎰 v2.0.875-P9-bet-double**: 同 symbol 同向上一筆 close 蝕 → 下一筆同向 size ×2(cap 0.20)——V3 邏輯實驗 296 筆三關全過(子集 EV +1.25%/筆 vs 全樣本 +0.74%、8/8 symbol、holdout +25.5pp、實盤可達 +140.7pp); `shouldBetDouble()`/`applyBetDoubleSize()` 純函數; 主神規則「贏咗恢復 1×」(prev pnl≥0 → false); env `BET_DOUBLE_ENABLED` 預設 off(等 P15 shadow 驗證先 enable)。

**完整結構（v2.0.855-audit + v2.0.875 更新）**：

| 元件 | 位置 | 作用 |
|:-----|:-----|:-----|
| `ShadowPosition` | interface | `side` / `entryPrice` / `stopLossPrice` / `takeProfitPrice` / `highSinceOpen` / `lowSinceOpen` / `mfePct` / `maePct` / `shadowType` / `factorTag` / `entryStats`(SCL 收據: WR/EV/verdict/OLR pwin/`entryBetDoubleEligible`) |
| `SHADOW_CONFIG` | constant | `maxOpenPerSymbol=10` / `maxTotalOpen=60` / **`maxOpenPerSide=30`(v2.0.875)** / `maxAgeCycles=12`(60min force-resolve) / `staleLearningWeight=0.3` |
| `openShadowTrades()` | blind | 每 cycle 為每個 trading market 開 LONG+SHORT 兩邊(cold-start prior，OLR weight 0.1×; **v2.0.875 受全局 per-side 配額**) |
...
| `openSeededShadow()` | seeded | 跌市強制播種 sell(persistent_bear + mom24h<0 + mom4h<0 雙確認)——**sell 樣本出口**(v2.0.875: 池滿可 evict qrl-buy) |
| `openQRLShadow()` | qrl | Q-RL expectancy lean(**v2.0.875: per-symbol ≤3 + 全局 ≤24 封頂——已證偽源唔可以壟斷**) |
...
- **四窗統一 + exit %（v2.0.870-fourwindow-unified）**：主神調查「回調震盪 keep BUY」（DRAM 24h 5 單 BUY 全蝕照開）——P79 四窗死貓彈（5m順+15m逆 → HARD BLOCK）原本只喺 active 主路徑行，per-symbol 開倉冇四窗 → 已接入 `applyEntryConvictionGates` 統一執行路徑（active + 所有 trading market）；**exit thesis 全面改用百分比**（盈虧/SL/TP/變動全部 % from entry + margin-basis %，一個 $ 金額都冇）；閘門回歸（Recent-loss 取消、OLR 0.35、額外閘移除、threshold 0.50 原樣）；exit 攻擊輪 A4 isBuySide 大小寫迴歸修復 + NaN% 防禦
