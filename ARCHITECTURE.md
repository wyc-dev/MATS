...
> **作者**: YC Wong · **版本**: 2.0.873-P9-attack-round4
> **核心哲學**: 資本保存為絕對第一優先，但必須在安全前提下持續創造盈利
> **測試狀態（v2.0.873-P9-attack-round4）**: vitest 3803 pass + 13 pre-existing fail（v2.0.854/868 時代，零新增）；另 9 個 legacy `node:test` 格式 file vitest 收集唔到 + 1 個測已剷代碼——開發噪音非 regression；`tests/p7-lyapunov-fix.test.ts`（P7，12 測試）本地有效（tests/ gitignored）；OLR hard gate 已知 2/3 接駁（active 主路徑只有 EV gate）——**P9-olr-audit 已取代（OLR 硬閘統計噪音 → 默認 OFF，env `OLR_HARD_GATE='true'` 可逆）**
