...
- **Live Mark price refresh**: the UI Mark was stuck at the Entry price because (a) HL `getPositions()` returns `entryPx` as `currentPrice` (never updated), (b) the `marketState` aggregator only has the ACTIVE symbol's ticker (can't price non-active position symbols), and (c) positions without a local mirror (e.g. BTC in `realPositions` + `cachedExchangePositions` but not `p.positions`) fell back to the stale `exPos.currentPrice`. Fix: `cachedPriceMap` caches live prices from `fetchPricesForSymbols` each cycle (all trading-market + open-position symbols); `refreshPositionMarkPrices()` (async, fire-and-forget from `pushToAPI`) updates each real position's currentPrice from the cache + on-demand fetches missing symbols; `serializePortfolio`'s `cachedExchangePositions` fallback uses `cachedPriceMap` live price (`localPos.currentPrice || cachedLive || exPos.currentPrice`). Verified: BTC + xyz:MU Marks now track the live price (SAME=False).

### v2.0.139 Conviction-gate feedback trap fix (Options B+C+D)

The adaptive-filter conviction gate had drifted to 59-64%, blocking all new entries despite strong Meta-Agent theses. Root cause: a feedback trap — thesis-invalidation force-closes counted as losses → winRate 0-10% → gate raised to `ceiling × 0.85 = 0.64` → new entries blocked → no wins to recover winRate → system stuck in cash. Three fixes:
- **C (root cause)**: `'thesis_invalidation'` added to `closeReason`; a `thesisInvalidatedCloseSymbols` Set marks force-closes before `closePosition` fires the `onPositionClosedLearning` callback (synchronous) → tags the `tradeHistory` record; `recentWinRate` computation filters out `closeReason='thesis_invalidation'` so the gate only tightens on real market-risk losses (SL hit), not thesis-system force-closes. Verified: winRate recovered 0-10% → 40%.
- **B (losing target lowered)**: `ceiling × 0.85`(0.64) → `ceiling × 0.70`(0.525). Real SL-hit losing streaks still tighten the gate but stay within the Meta-Agent's producible conviction range (55-65%) → re-entry remains possible.
- **D (automatic reset)**: filters construct at `floor + 0.10` (0.50) on startup; with C excluding thesis-invalidation losses, winRate no longer triggers the losing branch → gate stays at default 0.50. Verified: thresholds dropped 59-64% → 50-55%.
- **Active-symbol conviction dilution** (commit f7e7b92): the active-symbol path used the Meta-Agent's top-level confidence (diluted by HOLD sub-agents) instead of the per-symbol `marketTicker.confidence`. Same fix as v2.0.132 for trading markets — now the gate sees the per-symbol conviction (e.g. 0.65, not 0.55).
...
**v2.0.140 EM Cycle Digestion**：每個 CycleSummary 嘅 `keyInsight` 被 MiniLM embed 成 384-dim 向量，存入獨立嘅 `insightVectors[]` store。新 cycle 開始時，當前市況描述被 embed 並 query 所有歷史 insight vectors → top-3 最相似嘅歷史 cycles 注入 Meta-Agent context（`=== HISTORICAL SIMILAR INSIGHTS ===`）。

**自我調整**：每個 trade close 時，win/loss outcome 被回饋到開倉 cycle 所 retrieve 嘅 insights。EMA-smoothed `insightAccuracy` 追蹤 retrieved insights 是否正確預測 outcome。accuracy 高 → insights 可信；accuracy 低 → 市場 regime 可能已變。

**持久化**：`em-state.json` 儲存最近 288 個 CycleSummaries + convergence stats。每個 cycle save 一次，shutdown 時 save。重啟時 load + `rebuildInsightVectors()` 重建 vector store。

**Backfill**：`scripts/backfill-em-state.ts` 從 `data/exp/trades.jsonl` 構建歷史 CycleSummaries，令 EM Cycle Digestion 喺冷啟動時已有歷史數據可 query。
...
### 永續儲存 (`persistence.ts`)
`lockedWrite()` atomic write（v2.0.1）。State files: `olr-state.json` · `shadow-state.json` · `trade-patterns.json` · `evolution-state.json` · `portfolio-state.json` · `market-agent-config.json` · `debate-history.json` · `em-state.json`（v2.0.140: EM Cycle Digestion summaries + convergence）。
...
**v2.0.140b tick-level vs cycle-level scale fix**：`priceHistory` 儲存 ~100 個 WS ticks（~10 秒數據）。Tick-level σ 自然極低（0.01-0.05%），但 `RegimeCalibrator` thresholds（volLow=0.3%, volHigh=3%）係為 cycle-level（5 分鐘）σ 校準嘅。呢個 scale mismatch 令所有 regime 永遠 `low_volatility`。修復：tick-level σ 乘以 √(cycleDuration / historyDuration) ≈ √30 ≈ 5.5，縮放為 per-cycle σ。呢個係 Experience Digestion 偵測到嘅「VOLATILITY ANOMALY: 26/26 trades 100% low_volatility」嘅真正根因。
