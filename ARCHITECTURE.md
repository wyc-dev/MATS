...
│   │
│   ├── analysis/
│   │   ├── sigmoid-ga.ts         # Sigmoid·GA 情緒感知引擎 (v2.0.1 — +fundingRateAccel)
│   │   │   ├── sigmoid()         # sigmoid 函數
...
│   │   ├── em-clustering.ts     # 🧬 第二層 EM — GMM Clustering Engine (v2.0.5)
...
| **GMM EM Clustering** | 發現盈利/虧損 trade 嘅隱藏集群 | Gaussian Mixture Model | `em-clustering.ts` |
...
### 第二層：GMM EM Clustering（隱藏集群發現）

> **目的**: 用 **Gaussian Mixture Model (GMM)** 透過 Expectation-Maximization 從 trade 嘅 feature vectors 自動學出隱藏集群，每個 cluster 代表一類市場條件下嘅交易行為，帶有自身嘅 win rate。
>
> 唔係 KNN（逐條比對歷史 trade），而係 **generative model**（學分佈，壓縮知識）。當 Agents 問「呢個 setup 似唔似贏錢嗰類 trade？」，EM clustering 直接俾 probabilistic answer。
>
> **位置**: `src/evolution/em-clustering.ts`

### 原理

GMM 假設數據由 K 個高斯分佈混合生成。EM 迭代找出最可能嘅分佈參數：

```
E-step:    計算每個 sample 屬於每個 cluster 嘅 probability (responsibility)
              P(cluster_j | sample_i) = π_j · N(sample_i | μ_j, Σ_j) / Σ_k π_k · N(sample_i | μ_k, Σ_k)

M-step:    用 responsibility-weighted 更新每個 cluster 嘅參數
              μ_j = Σ_i r_ij · x_i / N_j
              Σ_j = Σ_i r_ij · (x_i - μ_j)(x_i - μ_j)^T / N_j
              π_j = N_j / N

Convergence: log-likelihood 變化 < 1e-6
```

### 特徵維度（11 維 Numerical）

與 `TradePatternContext` 的 numerical features 一致，全部 z-score 歸一化：

| 維度 | 意義 |
|:-----|:------|
| `volatility` | 24h 波動率 |
| `trendStrength` | 趨勢強度 0-1 |
| `srDistanceBps` | 離 S/R 距離 |
| `obImbalance` | 訂單簿傾斜 |
| `sentiment` | Sigmoid·GA 情緒 |
| `signalAgreement` | Agent 共識度 |
| `fundingRate` | 資金費率 |
| `fundingRateAccel` | 資金費率加速度 |
| `volumeRatio` | 成交量異常 |
| `positionSizePct` | 倉位大小 |
| `sentimentConviction` | GA 信心度 |

### BIC 模型選擇

系統自動嘗試 K=2..6 clusters，用 **Bayesian Information Criterion (BIC)** 選最佳：

```
BIC = -2 · log-likelihood + K · (2d + 1) · log(N)
    其中 d = 11 (特徵維度)
```
...
```
每個 meaningful trade close（|PnL| >= 0.5%）:
  → feedTrade(feature_vector, outcome)
  → 累積 50 個新 trade → auto-refit

Refit:
  → 對所有 pending samples 跑 EM（K=2..6, BIC 選最佳）
  → 每個 cluster 計算 weighted 實際 win rate
  → 儲存 model 到 data/evolution/em-clusters.json
  → 清除 pending buffer（bound memory）
```

### Query

```
query(features) → EMQueryResult:
  responsibilities[]      — soft cluster assignment (0-1 per cluster)
  weightedWinRate          — cluster-weighted win rate expectation
  dominantCluster          — argmax 嘅 cluster index
  zScores[]               — 每個維度離 cluster centroid 嘅標準差距離
  explanation              — human-readable summary
```

### Agent Context 注入格式

每個 `formatEntryContext()` 嘅輸出自動包含 EM assessment：

```
=== BUY ENTRY PATTERN INSIGHTS ===
...（原有 KNN 結果）...

EM Cluster: #2 (83% assignment)
  Weighted win rate: 72% (cluster-weighted expectation)
  🟢 EM favours this trade
```

### 方向決策 Priority Chain（Updated for EM）

| Priority | 信號 | 條件 |
|:--------:|:-----|:------|
| **1a** | Pattern DB (KNN) | `adjustedWinRate` via Wilson score, >=3 matches |
| **1b** | **GMM EM clustering** | **cluster-weighted win rate, dominant cluster > 50%** |
...
| 檔案 | 用途 |
|:-----|:------|
| `src/evolution/em-clustering.ts` | GMM EM 引擎 — E-step, M-step, BIC, online norm, context formatting |
| `src/evolution/trade-pattern-classifier.ts` | 整合 EM `.em` 屬性，backfill 時 feed trade，query 時 output assessment |
| `data/evolution/em-clusters.json` | 持久化 EM model（mean, covar, weight, winRate per cluster） |
...
```
Priority 1: Pattern Classifier
  └─ queryEntry('buy') vs queryEntry('sell') → adjustedWinRate 比較
  └─ 需要 >=3 meaningful matches 先算（Wilson score 防小樣本 overfit）

Priority 2: Sigmoid·GA Sentiment
  └─ overallSentiment > +0.15 → BUY
  └─ overallSentiment < -0.15 → SELL
  └─ |overallSentiment| <= 0.15 → 中性，fall through

Priority 3: Funding Rate
  └─ rate < -0.01% → BUY（longs get paid）
  └─ rate > +0.01% → SELL（longs pay premium）
  └─ |rate| <= 0.01% → 中性，fall through
...
Priority 5: Regime / Trend
  └─ trending_bull → BUY
  └─ trending_bear → SELL
  └─ other regimes → 中性，fall through

Default（所有信號中性）→ BUY（中性方向）
```
...
### Context Vector（13 維特徵 + Categorical Regime）
...
| **`fundingRateAccel`** | **8%** | **資金費率加速（-1~+1，new in v2.0.5）** |
| `signalAgreement` | 7% | Agent 共識度 |
| `fundingRate` | 6% | 資金費率靜態值 |
| `volumeRatio` | 6% | 成交量異常 |
| `positionSizePct` | 4% | 倉位大小 |
| **`sentimentConviction`** | **3%** | **GA 情緒信心度（0-1）** |

**v2.0.5 改動：**
- 新增 `fundingRateAccel`（權重 8%）— 資金費率加速度，用嚟捕捉 funding rate 嘅趨勢（正數 = funding 緊急上升，利淡長倉）
- `signalAgreement` 由 8% 降至 7%，`fundingRate` 由 8% 降至 6%，`volumeRatio` 由 8% 降至 6%，`positionSizePct` 由 5% 降至 4%，`sentimentConviction` 由 5% 降至 3%
- **GMM EM Clustering 整合**：餵入 meaningful trades 訓練 GMM，queryEntry() 輸出 cluster-weighted win rate 做方向參考
- **方向決策 Priority Chain**：Pattern(KNN) → EM(cluster-weighted) → Sentiment → Funding Rate → OB Imbalance → Regime
...
Priority 1b: GMM EM Clustering — cluster-weighted win rate
...
- ✅ Pattern Classifier (KNN + GMM EM) 為最高優先級
...
│  │   ├─ 第二層: GMM EM Clustering (11 維 GMM, BIC 選 K, cluster-weighted win rate)   │
...
│  ├─ P0 Trade Pattern Classifier (v2.0.5 — FR Acceleration + Wilson Score + GMM):                           │
...
│  │   ├─ 13 維 context vector (含 fundingRateAccel) + transition similarity │
│  │   ├─ Backfill: scripts/backfill-patterns.mjs → 歷史 trades import      │
│  │   ├─ 整合 GMM EM Clustering → query 輸出 cluster-weighted win rate    │
