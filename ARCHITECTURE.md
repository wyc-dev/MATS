...
| 條件 | 說明 |
|:-----|:-----|
| 倉位 > 20% | 單筆交易超過投資組合 20%（與 HACP clamp 一致） |
| 止損虧損 > 2% | 止損觸發將導致 >2% 投資組合虧損 |
| 回撤 > 20% | 總回撤超過 20% |
| 日虧損 > 5% | 當日虧損已達上限 |
| 制度 = 混沌 | 市場處於不可預測狀態 |
| 風險回報比 < 1:2 | 潛在虧損超過潛在利潤的 2 倍 |
...
在 HACP 引擎中加入 **自動硬上限 clamp**，在所有 Agent 輸出決策後、Risk Auditor 審計前執行：

```typescript
const MAX_POSITION_PCT = 0.20; // 20% 硬上限

function clampPositionSize(decision: TradingDecision): TradingDecision {
  if (decision.positionSizePct > MAX_POSITION_PCT) {
    decision = { ...decision, positionSizePct: MAX_POSITION_PCT };
    log.warn(`Position size clamped: ${original}% → 20%`);
  }
  return decision;
}
```
...
| 觸發點 | 位置 | 說明 |
|:-------|:----:|:-----|
| Meta-Agent 仲裁結果 | `metaAgentArbitration()` | 最終仲裁決定輸出前 clamp |
| 加權共識結果 | `buildConsensus()` | 多數決共識輸出前 clamp |
| Risk Auditor 硬限制 | `riskAuditorAudit()` | 硬上限檢查同樣使用 20%（保持一致） |

### 效果

- Meta-Agent 輸出 `positionSizePct: 2.0`（200%）→ 自動降至 `0.20`（20%）
- Risk Auditor 唔會因為 20% 而 veto（上限一致）
- Log 記錄 clamp 事件：`Position size clamped: 200% → 20%`
- 所有 Agent 輸出統一經過 clamp，避免 LLM 不一致性
...
### Context Vector（10 維特徵）

| 特徵 | 權重 | 意義 |
|:-----|:----:|:------|
| `regime` | 40% (categorical) | 制度匹配最關鍵 |
| `volatility` | 20% | 高波動 vs 低波動 |
| `trendStrength` | 15% | 趨勢強度 |
| `srDistanceBps` | 15% | 離 S/R 多遠 |
| `obImbalance` | 15% | 訂單簿傾斜 |
| `signalAgreement` | 10% | Agent 共識度 |
| `fundingRate` | 10% | 資金費率 |
| `volumeRatio` | 10% | 成交量異常 |
...
每 3 個決策週期，若 consensus 為 HOLD 且無持倉，系統強制開一個 1% 微型倉位：
...
if (finalDecision.action === 'hold' && this.totalCycles % 3 === 0) {
  if (p.positions.size === 0) {
    ...
    finalDecision = {
      ...
      symbol: 'BTCUSDT',
      positionSizePct: 0.01,    // 1% — 極小風險
      stopLossPct: 0.01,        // 1% 止損
      takeProfitPct: 0.02,      // 2% 目標
      rationale: `Exploratory ${direction} (1% size)`,
      urgency: 'immediate',
    };
  }
}
