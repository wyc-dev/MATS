...
| SELL 追空弱入場(above demand / trending_bull 字面) | **WORKING — DO NOT RE-DIAGNOSE / RE-IMPLEMENT** | `thesis-experience.ts` `computeChasePenalty(side, thesis)` + EXP gate 接駁 L1102 | v2.0.873-P9-SE-chase(60125bd)+ v2.0.875-SE-reflection — SELL + thesis 含 `above demand` → pWin×0.8; thesis 含 `trending_bull` → pWin×0.85（soft ≤20%, 取較大者唔疊加; 數字格式「165bps above demand」照 match——regex substring）。SKHX 2/2 全蝕實錘。⚠️ 樣本 <3 唔准再加碼 / 改簽名（TS2554 又嚟）/ 加 regime 參數——L1673 註解 pending 構想等樣本。 |
| 任何「premature_sl → widen SL」變體(regime-aware / volatility-scaled / execution-lens) | **EXTINCT(永久否決)** | — | v2.0.874-SL-widen-experiment — 21 筆 sl_tp 零 look-ahead counterfactual: widen 1.5× Σ=−216pp vs 原 −158pp 更差、好轉僅 5/21; 76% SL 係正確止蝕(被打後價格繼續走輸)。SKHX 09-08/09-09 係 trending_bull 追空(方向錯)唔係 SL 太緊——widen 令 −8% 變 −16%。任何 SL-widening flavor 直接引用 CHANGELOG v2.0.874 否決, 唔准換名重做。 |
...
| No exploration strategy (pure exploitation) | **NEW (paused)** | `active-exploration.ts` | v2.0.219 — UCB score = pWin + c·sqrt(ln(N_total)/N_symbol). Info gain boost when Bayesian uncertainty high. Annealing. Soft gating (never hard-blocks). ⚠️ PAUSED v2.0.833 (`ACTIVE_EXPLORATION_ENABLED=false`) — blind UCB without validated edge is dangerous. Re-enable after Edge Report proves baseline edge |
| World model (latent imagination) | ⛔ **REMOVED v2.0.833** | `world-model.ts` | v2.0.219 created, v2.0.833 removed (identity transition model, 0 `predict`/`rollout` call sites). File on disk. Do NOT re-add wiring |
...
| world-model / reward-shaping / cross-symbol / temporal-attention (dead inference) | **REMOVED** | `index.ts` (unwired) | v2.0.833 — 4 components had training wired but 0 inference call sites. Removed imports, fields, constructors, loads, feeds, saves, stats. Files remain on disk. Do NOT re-add their wiring. |
| active-exploration blind UCB | **PAUSED** | `index.ts` | v2.0.833 — `ACTIVE_EXPLORATION_ENABLED=false` default. Blind exploration without validated edge is dangerous. Do NOT re-enable without Edge Report proving baseline edge. |
| Edge validation layer | **NEW — DO NOT MODIFY** | `src/edge/*` | v2.0.833 — 6 modules: edge-calculator, execution-tracker, stability-monitor, risk-profile-edge-store, backtest-validation, edge-config. Alpha "lie detector". SE scope FORBIDDEN (see Scope). |
| Prototype pollution in edge weights lookup | **FIXED** | `src/edge/edge-calculator.ts` | v2.0.833 — `Object.hasOwn` defends against `regime='__proto__'` returning Object.prototype. Do NOT revert to plain `weights[key]`. |
| Cold-start deadlock from skipEdgeReport | **FIXED** | `src/edge/edge-calculator.ts` | v2.0.833 — `skipEdgeReport` returns `caution` + `0.5` (not `skip` + `0`). A brand-new system with zero trades must NOT be blocked — it needs to trade to accumulate samples. Ignorance ≠ evidence of no-edge. Do NOT revert to `skip`. |
...
11. **Close-context-aware learning — always pass `slNarrowed` + `learningWeight` to `feedTrade` (v2.0.226).** How a position is closed is an important factor in the loss. `computeLearningWeight(closeReason, slNarrowed, isWin)` must be called for every close-learning feed. Never call `feedTrade` with only 6 params (slNarrowed defaults false, weightMultiplier defaults 1.0 — all losses at full weight = contamination). Tight-SL losses (SL narrowed post-entry) are execution problems, not entry problems — they must be downweighted to 0.3. Combo WR tracker must skip losses with `learningWeight < 0.5`. `feedAdvancedLearning` must scale PnL by `learningWeight`.
...
- **Replay Buffer** (`replay-buffer.ts`, v2.0.219): Ring buffer (capacity 10000 as of v2.0.833) stores all trade records. `add()` computes priority from |pnl|. `replayEpoch()` samples mini-batch via Prioritized Experience Replay (Schaul et al. 2015) — `p_i = priority_i^α / Σ`, IS weights `w_i = (N·p_i)^(-β)` correct PER bias. Re-feeds OLR with `weightMultiplier = min(isWeight, 5)`. Cold-start guard (< 10 samples → no-op). Atomic save/load.
- **Bayesian OLR** (`bayesian-olr.ts`, v2.0.219): Wraps OLREngine with MC Dropout (Gal & Ghahramani 2016). `query()` runs N=30 forward passes with feature dropout (default 10%) → mean, std, 90% CI, epistemic uncertainty [0,1]. Seeded RNG (xorshift32) for reproducibility. Cold-start: < minSamples (20) → point estimate + uncertainty=1. Does NOT modify OLR internals — pure wrapper. ⚠️ PAUSED v2.0.833 with active-exploration.
- ⛔ **Temporal Attention** — REMOVED v2.0.833 (0 `retrieve()` call sites). File on disk, unwired. AttnRes cycle-history covers the time dimension.
- ⛔ **Cross-Symbol Backbone** — REMOVED v2.0.833 (0 `query()` call sites). File on disk, unwired. Per-symbol OLR + backfill covers cold-start.
- ⛔ **Reward Shaping** — REMOVED v2.0.833 (0 `shape()` call sites). File on disk, unwired. `learningWeight` (v2.0.226) covers execution-loss downweighting.
- **Active Exploration** (`active-exploration.ts`, v2.0.219): UCB `score = pWin + c·sqrt(ln(N_total)/N_symbol)`. Info gain bonus when Bayesian uncertainty > threshold (0.5). Annealing: exploration constant decays exponentially after annealingThreshold (500 trades). Soft gating only — never hard-blocks. ⚠️ PAUSED v2.0.833 (`ACTIVE_EXPLORATION_ENABLED=false`) — blind UCB without validated edge is dangerous. Re-enable after Edge Report proves baseline edge.
- ⛔ **World Model** — REMOVED v2.0.833 (identity transition model, 0 `predict`/`rollout` call sites). File on disk, unwired. A real world model needs separate entry/close features + sequence model.
...
> 即: SE 唔需要俾人話「你唔准做乜」——佢需要嘅係「判斷乜嘢先係真 edge / 邊啲改動有盈利動機」嘅框架。

### 每提出一個診斷/改動前——先自問三層級（強制）

```
層級 1 — ALPHA 層（真正盈利動機）:
  方向 lean / 訊號選擇性 / edge 使用率 / 期望值正負
  ——「系統係咪喺正確方向開倉?」「有 edge 但係咪有執行?」「SELL 偏斜?買 dip 用咗未?」
  ——改嘅係「點樣賺錢」（呢度先有 alpha——方向/選擇/edge）

層級 2 — 執行層（參數微調——唔會創造 alpha）:
  SL/TP 寬窄 / hold time / size 比例 / cooldown 時長
  ——「SL 距離」「volatility-scaled」呢類 = 執行優化——最多減少執行摩擦——唔會由「蝕」變「賺」
  ——如果 root cause 係「開錯方向」,執行層改動（如拉闊 SL）只會令蝕更多

層級 3 — 觀測/歸因層（記錄——唔係決策）:
  歸因統計 / GOT / reflection 記錄
```

### EDGE 判定三要件（改動要有 edge 先有盈利動機）
1. **有分辨力**: ρ / hit rate / counterfactual 支持（唔係「覺得啱」——係數據話「呢個訊號分到贏蝕」）
2. **影響方向 lean**: 改到「系統向邊個方向開倉」——而唔係「同一方向點執行」
3. **改變期望值**: counterfactual/重播放——正期望先做

### 盈利動機檢查（改動做完要能答）
- 「呢個改動令系統**邊度**賺多咗?」——方向 lean 準咗?edge 用多咗?期望正咗?
- 答唔到「邊度賺」→ 個改動唔係盈利動機（可能係執行微調/自覺良好）——唔值得做
