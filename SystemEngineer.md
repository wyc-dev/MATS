...
12. **Plan G dynamic threshold — penalties are MULTIPLICATIVE, not additive (v2.0.227).** The conviction gate uses a `DynamicThresholdCalculator` (`src/analysis/dynamic-threshold.ts`) that computes a dynamic threshold [45%, 55%] from 5 objective performance factors with hysteresis, and a multiplicative `penaltyFactor` with idle-based decay. NEVER re-introduce additive penalty-on-threshold (the death spiral: 50% + 30% = 80% threshold vs 44.5% confidence = impossible). The effective confidence formula is `consensus × pwinBlendFactor × penaltyFactor ≥ dynamicThreshold`. Penalties decay linearly over 30 idle cycles (self-recovery). 6 fairness guarantees: multi-factor balance, symmetric design, sample-size requirement, hysteresis, hard cap [45%, 55%], fact-driven.
