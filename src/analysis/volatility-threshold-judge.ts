...
## 輸入(每個 asset):
- 資產類型(加密貨幣/貴金屬/指數/股票)
- 歷史波動率分布(5 分鐘 σ——p25/median/p75/max——小數)
- 當前市場狀態(regime/trend/volatility)
- 新聞/宏觀摘要(如果有)
...
2. 用「歷史波動率分布」校準(數據驅動):
   ...
   - 如果歷史分布顯示「median 0.2%」——volLow 應該 < 0.2%(唔係 0.3%)

3. 動態調整(市場百變):
   - 市場波動大(新聞/宏觀事件/高 volume)→ threshold 提高
   - 市場平靜(低 volume/無新聞)→ threshold 降低
