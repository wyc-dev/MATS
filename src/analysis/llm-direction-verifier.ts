...
export class LLMDirectionVerifier {
   ...
   *  v2.0.865-fix7c(主神裁決):移除「賠率感知」EVFactor——
   *  ① 多餘:EV Filter 已從 real/paper 歷史 trade 存 pnlPct → computeEV
   *     計真 avgWin/avgLoss/EV(真 EV)——Direction Verifier 再加 = 重複
   *  ② 之前嘅 EVFactor 用 accuracy proxy(假設 avgWin=avgLoss)= 假貨
   *  ③ 主神:贏錢就足夠,風險用戶/管理員自己衡量
   *  → getTrustMultiplier 純 accuracy 乘數 */
