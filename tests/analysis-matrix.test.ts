...
import type { AggregatedMarketState as MS } from '../src/data/binance-websocket.ts';
...
describe('AnalysisMatrixBuilder — v2.0.822', () => {
  ...
  it('aggressive profile: conviction scaled ×1.3 (capped 1.0), calibrated=false', () => {
    const a = buildAssetAnalysis('btc', psc('buy', 0.6), ms(50000), 104, 0.6, 9, 11)!;
    expect(a.matrix.aggressive.flat.action).toBe('buy');
    expect(a.matrix.aggressive.flat.conviction).toBeCloseTo(0.78); // 0.6 × 1.3
    expect(a.matrix.aggressive.flat.calibrated).toBe(false);
  });

  it('aggressive conviction caps at 1.0', () => {
    const a = buildAssetAnalysis('btc', psc('buy', 0.9), ms(50000), 105, 0.6, 9, 11)!;
    expect(a.matrix.aggressive.flat.conviction).toBe(1.0); // 0.9 × 1.3 = 1.17 → cap 1.0
  });

  it('conservative profile: conviction scaled ×0.7, calibrated=false', () => {
    const a = buildAssetAnalysis('btc', psc('buy', 0.8), ms(50000), 106, 0.6, 9, 11)!;
    expect(a.matrix.conservative.flat.action).toBe('buy');
    expect(a.matrix.conservative.flat.conviction).toBeCloseTo(0.56); // 0.8 × 0.7
    expect(a.matrix.conservative.flat.calibrated).toBe(false);
  });
  ...
  it('all 9 cells exist and have valid action enums', () => {
    ...
    for (const profile of ['aggressive', 'moderate', 'conservative'] as const) {
      for (const state of ['long', 'short', 'flat'] as const) {
        const cell = a.matrix[profile][state];
  ...
  it('ODP-6 fix: NaN/negative confidence clamped to 0, >1 clamped to 1', () => {
    ...
    // aggressive caps at 1.0 even with confidence=1
    expect(a3.matrix.aggressive.flat.conviction).toBe(1.0);
  });
