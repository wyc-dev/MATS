...
**Version**: 2.0.873-P9-sym-sell-unblock · **Tests**: ~4,000 total (vitest, gitignored — 4011 pass / 13 pre-existing failures in v2.0.854-attack2-nan-price.test.ts + v2.0.868-attack, unrelated; 另 9 個 legacy `node:test` 格式 file vitest 收集唔到「No test suite found」+ 1 個測已刪代碼嘅死 file——開發噪音，非 regression，主神裁決唔使理) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~74,500 lines TypeScript (src 全樹) + legacy React UI (now superseded by `mats_app`)

- **v2.0.873-P9-got-observe**（主神 2026-09-01「提升盈利調查」）: ①純調查四候選三否決（swing-SL 全樣本重放 Δ−26~−46%——倖存者偏差教訓；deep-consensus defer Δ−19~−61%；SL-noise 非單調；美洲時段 BUY 兩半不穩——831.md §13）②**GOT 歸因修復**——entry gates 唔再記 generic 'gate' bucket（blockedBy 傳真 gate 名）；close-defer 記錄統一 source site（direction='close'+side），flush 唔再記 'hold' rows（滅雙重計數+語義污染）；legacy 'gate' pending 丟棄；`summary()` per-gate hit rate 每 100 cycle log——低 hit rate gate 進 P9-deadweight 停用候選。測試 3991 pass + 13 pre-existing。
- **v2.0.873-P9-sym-sell-unblock**（主神 2026-09-02「BTC & BNB 咁耐無開倉」——fix #1+#2, 先證後改）: ①**#1 activeSymbol 注入覆蓋**——activeSymbol=nonPositionMarkets[0]（btc）但 agents marketTicker 跟 selectedSymbol（UI 鎖定後 ≠ activeSymbol）,舊 `_additionalMarkets = nonPositionMarkets.filter(s => s !== activeSymbol)` 令 BTC 兩頭跌出 agents 視野（唔係 marketTicker + 唔喺 additionalMarkets → 8 日零 quote）;修復: `_additionalMarkets = nonPositionMarkets` 全保留（existingSyms dedup + perSymbolMap Map merge 防重複）。②**#2 Shadow SELL 樣本餓死**——5 路徑上限全部「全 symbol 合計 10」（buy 佔滿擋 sell, 死亡螺旋重現——實證 open buy 56/sell 4）;修復（方案 C）: 新 `countOpenBySide` helper 5 路徑改 per-side 上限（buy/sell 各 10）+ seeded sell bypass per-symbol 上限 + `evictOldestBlindForRoom(preferSide)` sell 優先 evict buy-blind;v2.0.861 不變式保留（blind 唔 self-evict）。資格不變式守住（persistent_bear + 雙 mom<0 先 seed——反彈型 BTC/BNB/GOLD 照舊唔 seed）。RED/GREEN 實驗 + 新測試 12/12;全量 4011 pass + 13 pre-existing（零新增）。
- **v2.0.873-P9-got-observe-attack**（主神 2026-09-01「不擇手段攻擊啱啱修葺嘅 code」）: GOT 攻擊輪——3 真漏洞全修: V1 HIGH `save()` 用 `fs.writeFileSync` 非原子寫（crash corrupt JSON → gate stats 全丟;其他學習組件已用 atomicWriteSync 唯獨 GOT 漏咗）→ `atomicWriteSync`;V2 MED load() 持久化污染（hits/misses=1e308 → n=Infinity → hitRate=NaN → comparator 不穩定 + NaN 輸出）→ clamp hits/misses ≤1e6、avgMovePct ≤±1e4;V3 LOW summary() NaN comparator → safe comparator + finite guard。**三問題驗證全屬實**: ①OLR 仲有投票權（olrBlendFactor 仍乘入 effectiveConfidence）②兩個 pwin 並存（signal 層 Shadow WR vs gate 層 OLR pwin）③shape/convexity 埋喺 base（LEDGER 拆解已解決）。測試 3995 pass + 13 pre-existing。
- **v2.0.873-P9-ledger-shape-expose**（主神 2026-09-01）: B（純觀測零風險）conviction ledger `base(...)` 拆 shape/convexity 成獨立條目（effectiveConfidence 數值零變——純歸因層）;A1 OLR blend 中性化 ❌（ρ=+0.019 但低 blend 組 −0.15% vs 全場 +0.51%——OLR 折扣有微弱選擇性,中性化 = 放返略差單）+ A2 Shadow WR 替換 ❌（E3 方向錯誤 + bnb 100% 集中 + outlier 驅動）——維持現狀。**⚠️ 更正**: 本座誤判「OLR 永遠 ≤50%」——嗰個係 81 子集,全樣本 OLR pwin 達 68.4%。Simpson's paradox 根因: symbol 層 ρ=+0.250 正確先驗 + within-symbol 噪音——OLR 係「弱但正確嘅 symbol 層風險過濾器」,唔係純噪音。測試 3995 pass + 13 pre-existing。
- **v2.0.873-P9-shadow-sym-case**（主神 2026-09-01「點解 Shadow WR 得 btc+bnb」）: Shadow WR 記錄 case-sensitivity bug——`getStats()` 返細階 vs 記錄處 `normalizeSymbol()` 保留資產名大小階（`xyz:SILVER` ≠ `xyz:silver`）→ 9/11 xyz: 資產 Shadow WR 靜默丟失（只有無前綴嘅 btc/bnb match 到）——**「Shadow WR 樣本太薄」結論係錯,係 bug**。修復: 新 `ShadowTradeEngine.getStatsForSymbol(symbol)` case-insensitive + null 防禦,index.ts 12 call site 統一。⚠️ 歷史 entryShadowWinRate 無法 retroactive 修復（look-ahead 污染——需等新數據 2+ 週先可重跑 66 counterfactual）。測試 3999 pass + 13 pre-existing。
- **v2.0.873-P9-sr-size-unify**（主神 2026-09-01）: sr-size-gate 統一入 `applyEntryConvictionGates`（方案 B）——原本只接 executeTrade（active 路徑),per-symbol consensus/exploration bypass（09-01 18:45 DRAM SELL −8.0% 追跌尾——S/R 距離 0.13% < 0.35% 應縮冇縮）;移除 executeTrade 舊 block（防 double-shrink）→ applyEntryConvictionGates shadow-gate 後統一執行（三路徑都行）;SELL-only + 純 size 層 + 冷啟動保守放行 + env `SR_SIZE_GATE=false` 回滾;`shouldShrinkSrSize` 單一 source of truth。⚠️ per-symbol threshold 唔做（data-snooping 陷阱——SKHX n=3 貼 S/R 反而好,831 §19）。測試 3999 pass + 13 pre-existing。
- **v2.0.873-P9-pnl-capture**（PNL 時段系列後續）: PNL 長頁面 capture/PDF——1 MONTH ~30000px 超 html2canvas 16384px 硬上限（全黑/空白/hang）;試錯 5 步（chunk revert/移除 windowWidth/動態 scale）→ **最終兩路方案**: 短頁面（≤10000px）保留 html2canvas+jsPDF（keep LOGO/QR）+ 長頁面（>10000px）用瀏覽器原生 `window.print()`（@media print 白底）。純 pnl.html UI 改動,零測試影響。
...
- **Multiplicative soft gates**: learning components scale confidence; NOTHING hard-blocks trades (owner directive).
  ...
  - ⚠️ **v2.0.873-P9-reentry-cooldown exception**: `shouldBlockChaseCooldown()` is a HARD 6h block on same-symbol same-direction ≥2 consecutive losses (DRAM 5× SELL defense; event-driven, NOT learning — experiment 47/49 Δ+123.8%, 三關全過). Owner-approved; do NOT extend to other gates without the same evidence bar.
...
- **v2.0.873-P9-verify-llm**（主神 2026-08-31 AI 預測能力驗證——純驗證零 code）: ①LLM + live context 事前判斷 60 筆——命中 47% < baseline 53%（deepseek-v4-flash 冇短期前瞻 + 開倉前資訊唔含未來——雙重限制）; ②HL API 歷史 tick 不可得（tradeHistory 唔通 / recentTrades 10 筆）——Live tick 驗證唯一路徑 = 現場收集 + GOT 閉環; ③「反序」揭穿——低 shadowWR +2.22% 剔 13 筆大 winner 後 −1.04%（尾部撐起——學反序=學噪音）。
...
- P9 deterministic entry-gate suite（`analysis/mom24-guard.ts` / `analysis/sr-size-gate.ts` / `analysis/boundary-align.ts` / `lib/reentry-cooldown.ts`——統一於 `applyEntryConvictionGates` + executeTrade,active/per-symbol/exploration 三路徑同一套）: mom24-guard（24h 弱勢 BUY block,維持 skip 版——大 winner 誤傷 12.9% 但淨 +88pp）+ chase-tail（mom24≥1.0% 但 last1<0 追升尾 block,同 mom24-guard 零重疊——0.5-1.0 緩衝）;sr-size（SELL 貼 S/R <0.35% → size×0.5 floor 0.01;三路徑統一 P9-sr-size-unify）;boundary-align（支尾 ≥4min → 延遲至下支開頭 ≤60s,純時序零 look-ahead）;reentry-cooldown（同 symbol 同向連蝕 N=2 → 6h hard block,同 exit_price_lock 1h 鎖利 cooldown 互補）。全數 env 可回滾;GOT per-gate summary 量度 hit rate,低命中進 P9-deadweight 停用候選。
