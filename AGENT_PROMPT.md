...
- **v2.0.873-P9-reentry-attack**（主神 2026-08-31「不擇手段攻擊」）: 連蝕 cooldown 攻擊輪——V1 CRITICAL `__proto__` prototype pollution 假造連蝕（`{__proto__:{streak:5}}` → streak 5+1 凍結開倉）→ state 讀取改 hasOwnProperty own-check 全修;Symbol/BigInt/時鐘/持久化污染/語義邊界 11 攻擊測試;量化提升——GOT 閉環（gate='reentry-cooldown' live hit rate 校準）+「過期後首單縮 size」假設推翻（首單 +3.17% 正——恢復交易正確, 避免負 EV 組件）;全量 3988 pass + 13 pre-existing（零新增）。
