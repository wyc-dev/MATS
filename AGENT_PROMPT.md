...
- **v2.0.873-P9-lock-pipeline**: 鎖利管道 margin-basis 校準（主神 2026-08-28「先驗證絕對成效,之後先 fix」）——驗證三層：269 喺解剖（WR 46.1% 但 PnL +1.53% 正期望值;145 蝕單 76% 曾浮盈 = 離場問題非入場問題）+ 保守可達分析（θ=0.5% margin → WR 85.5%）+ 前向 candle 重放無 look-ahead（θ=0.5% → WR 72.9%、PnL Δ+53.21%、誤鎖大 winner 0）。根因：鎖利門檻 price-basis 0.5% = margin 5% @10x, 71/110 可救單漏走。修復：`shouldColdStartLock` 加 leverage（margin-basis 0.5% 門檻, env `PROFIT_LOCK_MARGIN_THRESHOLD_PCT` 可調, clamp [0.01,5]）+ L3 trailing 前置 gate 同改 + lev 垃圾當 1 攻擊防禦;幻覺修正/確認式鎖掛/SL 唔收窄不變。
