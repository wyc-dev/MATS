...
**Version**: 2.0.870-P21 · **Tests**: ~2,647 total (152 suites; vitest, gitignored — 2634 pass / 13 pre-existing failures in gitignored v2.0.854-attack2-nan-price.test.ts + D4, unrelated) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~72,000 lines TypeScript (src 全樹) + legacy React UI (now superseded by `mats_app`)
