...
- **v2.0.870-P6 entry-quality**: 三方法入場質素——SL Floor（`RISK_STOP_LOSS_PCT` 0.008→0.015）+ Breakout 確認（`shouldSkipBreakoutEntry`——BUY 喺阻力位下方 < 50bps → skip）+ OLR 硬閘（`checkOLRHardGate`——OLR P(win) < 30% → block，`OLR_HARD_FLOOR`）。45 單 counterfactual 零贏單受影響。
...
- **v2.0.872-P8-transparency**: conviction 乘數總帳（convLedger——19 個 effectiveConfidence 乘數站位全部入帳，Plan-G 顯示/audit gates/asset_analyses metadata 全鏈透明）+  補 /（AutoTrade 落單必需，之前寫入 asset_analyses 時被剝）+ mats_web_app types mirror 同步。
...
- **v2.0.871-P7-audit（已知缺口，主神裁決暫唔修）**: OLR hard gate（`checkOLRHardGate`）只接咗 exploration + per-symbol 兩條入場路徑，**active finalDecision 主路徑只有 EV gate 冇 OLR gate**（index.ts ~12675）——主路徑仍有 conviction 校準 + verifier + threshold 多層保護，風險低，但同「三路徑同一套 gate」意圖有偏差。10 個 legacy `node:test` 格式 test file vitest 收集唔到（file-level FAIL 噪音）——測嘅係舊代碼舊行為，新 vitest 攻擊測試已全面覆蓋同組件，唔使理。
