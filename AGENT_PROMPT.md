...
**Version**: 2.0.869-P12 · **Tests**: ~2,522 total (141 suites; vitest, gitignored — 2509 pass / 13 pre-existing failures in gitignored v2.0.854-attack2-nan-price.test.ts, unrelated) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~63,000 lines TypeScript + legacy React UI (now superseded by `mats_app`)
