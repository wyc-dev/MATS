...
- **v2.0.873-P9-reentry-cooldown**（主神 2026-08-31「DRAM 5 次 SELL 如何防」）: 同 symbol 同向連蝕 hard cooldown——結構斷裂（regime switch）偵測。核心定理（47/48/49 實驗）: 平均統計（14d decayed）捕捉唔到結構斷裂（48 lean Δ−40.4% ❌——轉捩點前樣本全舊 regime）,「連蝕事件」先係即時偵測器（47/49 N=2×6h Δ+123.8% 三關全過）。merge 入現有 `lib/reentry-cooldown.ts`（同「鎖利 1h cooldown」互補——嗰個防贏單鎖完追, 呢個防蝕單連蝕追）;`updateCooldownOnClose` + `shouldBlockChaseCooldown` + index.ts（cooldownTracker persist + close 後更新 + 開倉前 block）;env `REENTRY_COOLDOWN_GATE/N/HOURS`。DRAM 案例: 2 連蝕→6h 停→01:12 −5.9 被抑。
