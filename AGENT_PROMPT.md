...
**Version**: 2.0.863 · **Tests**: ~2,050 total (86 suites; vitest, gitignored — full suite 2037 pass / 12 pre-existing failures in gitignored v2.0.854-attack2-nan-price.test.ts, unrelated) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~63,000 lines TypeScript + legacy React UI (now superseded by `mats_app`)
