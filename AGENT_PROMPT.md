...
**Version**: 2.0.869-P4 · **Tests**: ~2,318 total (130 suites; vitest, gitignored — 2280 pass / 14 pre-existing failures in gitignored v2.0.854-attack2-nan-price.test.ts, unrelated) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~63,000 lines TypeScript + legacy React UI (now superseded by `mats_app`)
