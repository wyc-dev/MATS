...
- **v2.0.873-P9-verify**（主神 2026-08-31 四輪驗證——純驗證零 code）: ①大 winner 審計——mom24-guard 誤傷 4/31（+52pp 尾部代價, net +88pp 維持）,近支撐/per-symbol（SKHX）豁免全推翻; ②sizing up ❌（全特徵反序——momentumLong/shadowWR/OLR「高信心」組全差——Post-Review「OLR edge size up」證實係已證偽建議源）; ③PLAN_5m-live-gate A 關閉（DRAM 開倉時支內順勢 gapToOpen<0——「開倉後反轉」preempt 唔到）+ 支內逆勢 gate 兩邊 ❌（BUY 剔 outlier Δ=0——outlier 幻覺）; ④誤傷成本審計——mom24-guard 1:1.84 / chase-tail 1:2.77 / sr-size 1:2.02（用 1 蚊盈利換 2-2.8 蚊虧損避免——健康）; 今日 counterfactual——sr-size 命中 2/6 淨 +2.55%（gate 係結構性改善唔係防今日）。全部實驗 scripts 38-45 + 831.md §9-12 + PLAN 檔（全 gitignore）記錄。
