...
**Version**: 2.0.870-P79 · **Tests**: ~2,900 total (186 suites; vitest, gitignored — 2983 pass / 13 pre-existing failures in gitignored v2.0.854-attack2-nan-price.test.ts + D4, unrelated) · **Build**: `tsc --noEmit` (zero errors) + `cd ui && npx vite build` (zero errors) · **Run**: `npm run dev` (concurrently runs API :3456 + UI :5173) · **Codebase**: ~74,500 lines TypeScript (src 全樹) + legacy React UI (now superseded by `mats_app`)
